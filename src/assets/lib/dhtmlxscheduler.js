...
dataProcessor.prototype = {
    ...
    scheduler._detachDomEvent = function(e, t, i) {
		...
		//console.log("scheduler._click calling");
		...
		//console.log("addEventNow calling");
        var r = {};
        ...
        if (this._drag_mode) {
            ...
            if (this._is_pos_changed(this._drag_pos, t)) {
                ...
                if (scheduler.matrix && (o = scheduler.matrix[scheduler._mode]), o = o || {
					//console.log("If Dargging calling..")
                        x_step: 1,
                        x_unit: "day"
		...
		//console.log("_on_mouse_down calling");
        if (2 != e.button && !this.config.readonly && !this._drag_mode) {
			//console.log("Inside _on_mouse_down calling");
            ...
            switch (i) {
                ...
                case "dhx_event_resize":
					//console.log("dhx_event_resize calling");
                    this._drag_mode = "resize";
                    var r = scheduler._getClassName(t);
					//console.log("dhx_event_resize calling", r);
            ...
            if (this._drag_mode) {
				//console.log("end of resize calling");
		...
		//console.log('updateView calling' );
        ...
        drag_highlight: !0,
        limit_drag_out: !1,
        icons_edit: ["icon_save", "icon_cancel", "icon_post"],
        icons_select: ["icon_details", "icon_edit", "icon_delete"],
        buttons_left: ["dhx_save_btn", "dhx_cancel_btn", "dhx_post_btn_set"],
    ...
    scheduler.templates = {}, scheduler.init_templates = function() {
        ...
        e.dhx_save_btn = e.icon_save, e.dhx_cancel_btn = e.icon_cancel, e.dhx_delete_btn = e.icon_delete, e.dhx_post_btn_set = e.icon_post;
        ...
        r(scheduler.templates, {
            ...
            event_bar_date: function(e, t, i) {
				//console.log("e event_bar_date",e);
				//console.log("t event_bar_date",t);
				//console.log("i event_bar_date",i);
    ...
    function() {
		...
		var lengthevent= false;
        for (var i = {}, r = 0; r < e.length; r++)
            if (this._table_view)
                if ("month" != scheduler._mode) this.render_event_bar(e[r], lengthevent);
                else {
                    ...
                    s !== 1 * s || e[r]._sorder < s ? this.render_event_bar(e[r], lengthevent) : void 0 !== s && e[r]._sorder == s && scheduler._render_month_link(e[r])
                } else {
		...
		//console.log("scheduler.render_event ");
        var r = scheduler.xy.menu_width,
            s = this.config.use_select_menu_space ? 0 : r;
        if (!(e._sday < 0)) {
			//console.log("inside scheduler.render_event ");
            var a = scheduler.locate_holder(e._sday);
            if (a) {
				//console.log("1 inside scheduler.render_event ");
                t = t || a;
				...
				setTimeout(function() {
					postEvent(e.text, e.start_date, e.end_date, e.id, e.location, e.description);
				}, 1000);
                ...
                if (this._waiAria.eventAttr(e, v), this._rendered.push(v), t.appendChild(v), c = c + parseInt(a.style.left, 10) + s, this._edit_id == e.id) {
                    v.style.zIndex = 1, _ = Math.max(_ - 4, scheduler.xy.editor_width), v = document.createElement("div"), v.setAttribute("event_id", e.id), this._waiAria.eventAttr(e, v), this.set_xy(v, _, o - 20, c, d + (scheduler.xy.event_header_height || 14)),
						...
						//console.log("FInal end dtart "+ e.start_date + ":: "+ e.end_date);
        ...
        E = l.firstChild;
		//console.log('Render event calling');
    ...
    }, scheduler.render_event_bar = function(e, len) {
        console.log("scheduler render calling..", len);
        if (len==true) {
            setTimeout(function(){
               $('.dhx_cal_event_line_start').unbind().click(function(){
                    var id = $(this).attr('event_id')
                    ShowEventOnRightSide(id)
                })
            },1000)
        }
        ...
        for (var r in this._events) {
			...
			//console.log('scheduler.getEvents calling', s);
    ...
    scheduler.formSection = function(e) {
        ...
        textarea: {
            render: function(e) {
                var t = scheduler._lightbox_controls.defaults.textarea,
                    i = t ? t.height : 200,
                    r = (e.height || i || "130") + "px";
                if( (e.name == "Title") || (e.name == "Location") )
                    return "<div class='dhx_cal_ltext' style='height:" + r + ";'><textarea onkeypress='return IsAlphaNumeric(event);' maxlength='75'></textarea></div>";
                else
                    return "<div class='dhx_cal_ltext' style='height:" + r + ";'><textarea onkeypress='return IsAlphaNumeric(event);'></textarea></div>";
            },
            ...
            _get_input: function(e) {
				//console.log("e.getElementsByTagName)[0]", e.getElementsByTagName("textarea")[0]);
        ...
        time: {
                ...
                for (var n = "", d = 0; d < t.length; d++) {
                    ...
                    switch (o) {
                        ...
                        case "%d":
                            ...
                            for (var c = 1; 32 > c; c++) if(new Date().getDate()<=c) h += "<option value='" + c + "'>" + c + "</option>";
                            break;
                        case "%H:%i":
            ...
            set_value: function(e, t, i, r) {
				//console.log('set value is calling');
                function s(e, t, i) {
        ...
        if (e) {
            ...
            $('.dhx_post_btn_set_set').show();
            $('.dhx_save_btn_set').attr("bid","");
            $('.dhx_save_btn_set').attr("mid","");
        ...
        if( ($('.dhx_save_btn_set').attr("bid") !="") && ($('.dhx_save_btn_set').attr("bid") != undefined))
        {
            if(e.text == "")
                showAlertEvent("event name");
            else if(e.location == "")
                showAlertEvent("location");
            else
            {
                updateEventWin(e.text, e.start_date, e.end_date, $('.dhx_save_btn_set').attr("mid"), e.location, e.description, $('.dhx_save_btn_set').attr("bid"), this._lightbox_id, "sender");
                $('.dhx_save_btn_set').attr("mid","");
                $('.dhx_save_btn_set').attr("bid","");
        ...
        {
            postEvent(e.text, e.start_date, e.end_date, this._lightbox_id, e.location ,e.description);
            (!this.checkEvent("onEventSave") || this.callEvent("onEventSave", [this._lightbox_id, e, this._new_event])) && (this._empty_lightbox(e), this.hide_lightbox())
        }
        ...
        this.getLightbox().onclick = function(e) {
            ...
            if (t.className || (t = t.previousSibling), !(t && t.className && scheduler._getClassName(t).indexOf("dhx_btn_set") > -1) || (t = t.querySelector("[dhx_button]"))) {
				...
				//console.log("Event Name calling>>>>", i);
                if (t && i) switch (i) {
                    case "dhx_save_btn":
                    ...
                    case "dhx_cancel_btn":
                        ...
                        break;
					case "dhx_post_btn_set":
                        if($("#input_"+eventID).val() == "")
                           showAlertEvent("event name");
                        else if($("#input_"+(eventID + 4)).val() == "")
                            showAlertEvent("location");
                        else
                            taskEventPost();
					break;
        ...
        //console.log("Get lightbox event");
        if (!this._lightbox) {
            ...
            //--------- for custome diesing -----------//
            //r = this._waiAria.lightboxButtonAttrString(i[1]), t += "<div " + r + " class='dhx_btn_set dhx_right_btn_set' style='float:right;' onclick=eventcustomPopup()><div dhx_button='1'></div><div>More option</div></div>";
            r = this._waiAria.lightboxButtonAttrString(i[1]), t += "<div class=dhx_wrap_section><div class=dhx_cal_lsection><label>Repeat</label></div><div class=dhx_cal_ltext><select onchange='eventcustomPopup();' id=customdropbox class=form-control custom-icon><option value=dnr>Does not repeat</option><option value=ed>Every Day</option><option value=ew>Every Week</option><option value=ey>Every year</option><option onclick='eventcustomPopup();' value=custom>Custom</option></select></div></div>";

            //--------- for custome diesing -----------//

            for (var s = 0; s < i.length; s++)
			{
                //console.log("Riht side list", i[s]);
    ...
    }), scheduler._lightbox_template = "<div class='dhx_cal_ltitle'><span class='dhx_mark'>&nbsp;</span><span class='dhx_time'></span><span class='dhx_title'></span><span class='dhx_close' style='color: #fff; float: right; cursor:pointer' onclick='closeDialogwindow()'>X</span></div><div class='dhx_cal_larea'></div>",
