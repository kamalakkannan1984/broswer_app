...
const fileSCOPES = ['https://www.googleapis.com/auth/drive.readonly', 'https://www.googleapis.com/auth/drive.metadata.readonly'];
...
jsxc = {	
    ...
    */
	selfPresence: function(bid, presence) {
		
		if (typeof(Storage) !== "undefined") {
			var key = "jsxc:"+jsxc.bid+":presence";  //jsxc:2171079@im03.vectone.com:presence
			localStorage.setItem(key,presence);
		}
		jsxc.gui.changePresence(presence,true);
		 //jsxc.changeState(presence);
	},
   ...
   restoreWindows: function() {
	   if (window.Debug) console.log("restoreWindows");
      ...
      $.each(windows, function(index, bid) {
		  if (window.Debug) console.log("restoreWindows-bid",bid);
   ...
   changeState: function(state) {
      ...
      $(".jsxc_windowItem").appendTo('#clientview');
	  if (window.Debug) console.log("state",state);
	  ...
	  if (state !== 'online') {
		 pres.c('show').t(state).up();
	  }
	 // var priority = jsxc.options.get('priority');
	  var priority = jsxc.options.priority;
	  if (window.Debug) console.log("priority",priority);
	  if (priority && typeof priority[state] !== 'undefined' && parseInt(priority[state]) !== 0) {
		 pres.c('priority').t(priority[state]).up();
	  }
...
jsxc.xmpp = {
    ...
    */
   attached: function() {
	 if (window.Debug) console.log("jsxc.xampp attach");
      $('#jsxc_roster').removeClass('jsxc_noConnection');
   ...
   sendPres: function() {
	  ...
	  if (window.Debug) console.log("sendPres01");
	 ...
	 if (window.Debug) console.log("send presence");
      ...
      if (priority && typeof priority[presState] !== 'undefined' && parseInt(priority[presState]) !== 0) {
	   ...
	   if (window.Debug) console.log("pres",pres);
      jsxc.debug('Send presence', pres.toString());
      window.presence = pres;
      jsxc.xmpp.conn.send(pres);
   ...
   onRoster: function(iq) {
	  ...
	  if (window.Debug) console.log("Load roster ", iq);
      ...
      $(iq).find('item').each(function() {
		 ...
		 if (window.Debug) console.log("bid ::: "+bid + ":: name ::: "+name + ":: bid :: "+bid)
		 
		 if (!jsxc.storage.getUserItem('buddy', bid)) {
			 if (window.Debug) console.log("inside storage added");
   ...
   onRosterChanged: function(iq) {
	   ...
	   if (window.Debug) console.log("ig01",ig);
   ...
   onPresence: function(presence) {
	...
	if (window.Debug) console.log("presence");
      /*
	  ...
	  if (window.Debug) console.log("Ptype ::: "+ptype);
      if (ptype === 'subscribe') {
         var bl = jsxc.storage.getUserItem('buddylist');

         if (bl.indexOf(bid) > -1) {
			 if (window.Debug) console.log("Auto approve contact request...");
      ...
      if (xVCard.length > 0 && data.type !== 'groupchat') {
		 ...
		 if (window.Debug) console.log("photo00",photo.text());
      }
   ...
   console.log('Got Now onChatMessage', stanza);
      var checkNote = ''; 
      var isEvent    =  false;
      var parentId   =  "";
      var noteDesc   =  "";
      if ( (stanza.getElementsByTagName('editfilename').length > 0) )
         return true;

         if ( (stanza.getElementsByTagName('like').length > 0))
         {
            var fromUser = ($(stanza).attr('from')).split("/")[0];
            var msgID = stanza.getElementsByTagName('like')[0].getAttribute('id');
            
            likemessage(msgID, fromUser, 'like', 'in', fromUser);
            return true;
         }

         if ( (stanza.getElementsByTagName('unlike').length > 0))
         {
            var fromUser = ($(stanza).attr('from')).split("/")[0];
            var msgID = stanza.getElementsByTagName('unlike')[0].getAttribute('id');
           
            likemessage(msgID, fromUser, 'unlike', 'in', fromUser);
            return true;
         }

         if ( (stanza.getElementsByTagName('pin').length > 0))
         {
            var fromUser = ($(stanza).attr('from')).split("/")[0];
            var msgID = stanza.getElementsByTagName('pin')[0].getAttribute('id');
           
            setFlagMode(msgID, true, fromUser,'received');
            return true;
         }
         if ( (stanza.getElementsByTagName('unpin').length > 0))
         {
            var fromUser = ($(stanza).attr('from')).split("/")[0];
            var msgID = stanza.getElementsByTagName('unpin')[0].getAttribute('id');
           
            setFlagMode(msgID, false, fromUser,'received');
            return true;
         }
		if (stanza.getElementsByTagName('composing').length > 0) {
            var chat = ($(stanza).attr('from')).split("@")[0]+"-ChatWind";
            var len = $("#ParentWindow").find("."+chat).length;
            if( len != 0 )
            {
               $("."+chat).find(".Compose_msg").css("display","block");
               setTimeout(function()
               {
                  $("."+chat).find(".Compose_msg").css("display","none");
               },2000);
            }
            return true;
        }
	...
	var isFile = false;
  var Parentid = "";
    if (stanza.getElementsByTagName('message-type').length > 0) {
         var check = stanza.getElementsByTagName('message-type')[0].getAttribute('value');


        var typeCheck = stanza.getElementsByTagName('message-type')[0].hasAttribute("type");
         var tagType = "";
        if(typeCheck)
            tagType = stanza.getElementsByTagName('message-type')[0].getAttribute('type');

         if(check == "NOTE")
         {
            checkNote = 'createnote';
            if (stanza.getElementsByTagName('replace').length > 0) { 
               try{
                  var notemsgdata = $(stanza).find('body:first').text();
                  var fromUser = ($(stanza).attr('from')).split("/")[0];
                  var Msgid = stanza.getElementsByTagName('replace')[0].getAttribute('id');
                  notemsgdata = JSON.parse(notemsgdata);
                  getNoteUpdate(Msgid, fromUser, "receiver", notemsgdata.UID, notemsgdata.SUMMARY, notemsgdata.DESCRIPTION, notemsgdata.DTSTART, function(data)
                  {
                     if(window.Debug)	console.log(data);
                  });
                  return true;
       ...
       else if(check == "TASK" || tagType == 'REPLYTASK')
      {
         var Taskdetailsdata = stanza.getElementsByTagName('body')[0].childNodes[0].nodeValue;

         if (stanza.getElementsByTagName('replace').length == 0)
            Parentid = stanza.getElementsByTagName('thread')[0].getAttribute('parent');

         console.log("msg Task", msg);

         taskdetails = JSON.parse(Taskdetailsdata);
         if (stanza.getElementsByTagName('replace').length > 0) { 
            console.log("replace", stanza);
            var fromUser = ($(stanza).attr('from')).split("/")[0];
            var Msgid = stanza.getElementsByTagName('replace')[0].getAttribute('id');
            updateParentTaskfromchat(Msgid, fromUser, taskdetails);
            return true;
         }
	 ...
	 else if(check == "EVENT")
	 {
      ...
      if (stanza.getElementsByTagName('deleted').length > 0) {
         var fromUser = ($(stanza).attr('from')).split("/")[0];
         var mid = stanza.getElementsByTagName('deleted')[0].getAttribute('id');
         checkDeleteMsg(mid, fromUser);
      }
      if (stanza.getElementsByTagName('data').length > 0) {
           var IsCustom = stanza.getElementsByTagName('data')[0].getAttribute('xmlns');
           if (IsCustom == "mycustom-data-ns") {
               var CustomData = stanza.getElementsByTagName('data')[0].getAttribute('data');
               var CustomOperation = stanza.getElementsByTagName('data')[0].getAttribute('operation');
			   var frm = $(stanza).attr('from');
                     frm = frm.substr(0, frm.indexOf('/')); 
                     
               plot_markings(CustomOperation, JSON.parse(CustomData), frm);
               return true;
           }
      }
	  ...
	  if (stanza.getElementsByTagName('edit').length > 0) {
			var xmlns = stanza.getElementsByTagName('edit')[0].getAttribute('xmlns');
			var uid = stanza.getElementsByTagName('edit')[0].getAttribute('id');
			var msg = stanza.getElementsByTagName('body')[0].childNodes[0].nodeValue;
			var fromUser = ($(stanza).attr('from')).split("/")[0];
			var id = uid.replace(":","-");
			$("#"+id).find(".msgTxt").text("");
			$("#"+id).find(".msgTxt").append('<b><span class="icon-edit weblinks"></span>'+msg+'</b>');	
			editMessageSave(uid, msg, fromUser);
			return true;
		}
    ...
    if (stanza.getElementsByTagName('copyquote').length > 0) {
         if (window.Debug) console.log("Quote is calling");
         var xmlns = stanza.getElementsByTagName('copyquote')[0].getAttribute('xmlns');
         var uid = stanza.getElementsByTagName('copyquote')[0].getAttribute('id');
         var msg = stanza.getElementsByTagName('copyquote')[0].childNodes[0].nodeValue;
         var timestamp = stanza.getElementsByTagName('copyquote')[0].getAttribute('quotedtime');
         mentionid   =   message.getElementsByTagName('copyquote')[0].getAttribute('mentionid');
         var id = uid.replace(":", "-");
		...
		var urlTitle	=	"";
		var summary		=	"";
		if(stanza.getElementsByTagName('urlLink').length > 0) {
			urlTitle	   = 	stanza.getElementsByTagName('urlLink')[0].getAttribute('title');
			summary		= 	stanza.getElementsByTagName('urlLink')[0].getAttribute('summary');
			urlLink		=	true;
		}			
      ...
      if (data === null) {
         ...
         var messageObj = new jsxc.Message({
            ...
            quoted_id : "",
            quoted_msg :"",
			   quoted_timestamp : quotedtimestamp,
            stamp: stamp
         });
         messageObj.save();
         return true;
      ...
      if ((htmlBody.length === 1) || (isFile)) {
		...
		if (jsxc.otr.objects.hasOwnProperty(bid) && body && isTask == false && isEvent == false && isFile == false && isquoted == false && urlLink == false && checkNote == '')  {
       ...
       if(isTask)
         {
            var t_name = taskdetails.SUMMARY
            jsxc.gui.window.postMessage({
               _uid: mid,
               bid: bid,
               direction: jsxc.Message.IN,
         ...
         else
         {
            ...
            data._uid = mid;
            data.bid = bid;
            data.direction = jsxc.Message.IN;
            data.msg = body;
            ...
            data.quoted_id = quotedmsg_id;
            data.quoted_msg = quotedmsg;
            data.quoted_timestamp  = quotedtimestamp;
            ...
            data.attachment = attachment
			   ...
			   jsxc.gui.window.postMessage(data);
         }   
    ...
    * @param {string} alias
    */
   addBuddy: function(username, alias, Ext, LoginId,MailId) {
		 if (window.Debug) console.log("jsxc.xmpp.con",jsxc.xmpp.con)
		if(jsxc.xmpp.conn != undefined)
		{
			if(username == Strophe.getBareJidFromJid(jsxc.xmpp.conn.jid))
				return;
		}
	   
	   
      var bid = jsxc.jidToBid(username);
	  if (window.Debug) console.log("bid",bid);
      if (jsxc.master) {
		  if (window.Debug) console.log("inside if");
         ...
         jsxc.storage.removeUserItem('add', bid);
			...
			if (window.Debug) console.log("inside else");
	  ...
	  if (window.Debug) console.log("before to local storage...");
	 if (!jsxc.storage.getUserItem('buddy', bid)) {
		 if (window.Debug) console.log("Adding to local storage...");
         jsxc.storage.saveBuddy(bid, {
               jid: username,
               name: alias,
			   rnd: Math.random(),
...
/*	var bl = jsxc.storage.getUserItem('buddylist');
	...
	if (window.Debug) console.log("add buddy list updating ", bl)
	if (bl.indexOf(username) < 0) {
	   bl.push(username); // (INFO) push returns the new length
	   
	   if (window.Debug) console.log("Updating list", bl)
	   jsxc.storage.setUserItem('buddylist', bl);
	   
	}
	*/
			
	  if (window.Debug) console.log("add buddy ended");
   },
   ...
   removeGroupWindow: function(groupName)
   {
		var bid = jsxc.bid;
		var own = jsxc.storage.getUserItem('ownNicknames') || {};
		var objkey = "jsxc:"+bid+":ownNicknames";
		...
		localStorage.setItem(objkey,JSON.stringify(own));
		own = jsxc.storage.getUserItem('ownNicknames') || {};
		
		var room =[];
		room = jsxc.storage.getUserItem('roomNames') || [];
		objkey = "jsxc:"+bid+":roomNames";
		if (window.Debug) console.log("key is",objkey);
		...
		localStorage.setItem(objkey,JSON.stringify(room));
		room = jsxc.storage.getUserItem('roomNames') || [];
		if (window.Debug) console.log("get from local server",room);
		
   },
   onReceived: function(stanza) {
    
    /*   console.log('Got Now onReceived', stanza); */
		if (stanza.getElementsByTagName('editfilename').length > 0) {
			var id = stanza.getElementsByTagName('editfilename')[0].getAttribute('id');
			var fileName = stanza.getElementsByTagName('editfilename')[0].getAttribute('filename');
			changeAttachmentName(id, fileName);
			return true;
      }
      if ( (stanza.getElementsByTagName('displayed').length > 0))
      {
			var id = stanza.getElementsByTagName('displayed')[0].getAttribute('id');
			var fromUser = ($(stanza).attr('from')).split("/")[0];
         getMesssageTick(id, fromUser, function(data)
         {
            console.log("return");
         })
         return true;
      ...
      if ( (stanza.getElementsByTagName('fileedit').length > 0))
      {
         var id = stanza.getElementsByTagName('fileedit')[0].getAttribute('id');
         var filename = stanza.getElementsByTagName('fileedit')[0].getAttribute('filename');
         var fromUser = ($(stanza).attr('from')).split("/")[0];
         editFileFameCallB(id, filename, fromUser, function(data)
         {
            //console.log("success");
         })
         return true;
      ...
      var gc = $(stanza).find("x");
      if(gc.length){
            if (gc[0].namespaceURI == 'jabber:x:conference'){
				var room = gc.attr('jid');
				//var id = gc.attr('id');
				//var from = $(stanza).attr('from');
               room = room.split('@')[0];
               jsxc.gui.queryActions.join(room,"New Room");
            }
      }
      ...
      if (received.length) {
		  if (window.Debug) console.log("received calling");
	...
	/*var fromUser = ($(stanza).attr('from')).split("/")[0];
      if(fromUser.includes("conference"))
         onGroupchatMessage(stanza);
      return true;*/
   },
   ...
   _sendMessage: function(jid, msg, message) {
      ...
      var type = (jid.includes("@conference")) ? 'groupchat' : 'chat';
      message = message || {};
		if (window.Debug) console.log("_sendMessage ", message);
	  ...
	  if(message.isTask)
		  msg = JSON.stringify(message.taskdetails);
   ...
   {
	   xmlMsg.up().c('edit', {xmlns: 'urn:xmpp:message-correct:0', id:message.editedmsg_id.replace("-",":")});
	   jsxc.storage.removeUserItem('msg', message._uid);
	   	$(".jsxc_textinput").html("");
		$(".jsxc_textinput").removeClass('editedmsg');
		$(".jsxc_editcancel").hide();
		$("#editeddid").val("");
   }
   
   if(message.quoted == true)
   {
	   xmlMsg.up().c("copyquote", {xmlns: "urn:xmpp:message-correct:0", id:message.quoted_id.replace("-",":"), quotedtime:message.quoted_timestamp,mentionid:message.mentionid}).t(message.quoted_msg);
		closeCopyQuoteDiv(message.quoted_id);	  
   }
  
   if(message.isTask)
	{
      
      //xmlMsg.up().c("message-type", {xmlns: "urn:xmpp:message-correct:0", value:"TASK"})
      //xmlMsg.up().c("thread", {parentid:(message.parentid) ? message.parentid : ""}).h(message.taskdetails.UID).up();	   
     // xmlMsg.up().c("thread",  {xmlns: "urn:xmpp:message-correct:0", parentid:message.taskdetails.THREAD_ID}).t(message.taskdetails.UID);
	}

   if(message.isEvent == true)
   {
      xmlMsg.up().c("message-type", {xmlns: "urn:xmpp:message-correct:0", value:"EVENT"})
      if( (message.parentid != undefined) && (message.parentid != "") )
         xmlMsg.up().c('replace', {xmlns: 'urn:xmpp:message-correct:0', id:message.parentid});
   }
   if(message.isFile == true)
   {
      xmlMsg.up().c("message-type", {xmlns: "urn:xmpp:message-correct:0", value:"FILE"})
      xmlMsg.up().c("x", {xmlns: "jabber:x:oob"})
      xmlMsg.c('url').t(message.msg).up();
   }


	if(message.like == true)
	{
		if (window.Debug) console.log("like message",message.likemsg_id);
	    xmlMsg.up().c("like", {xmlns: "urn:xmpp:message-correct", id:message.likemsg_id})
	}
	else if(message.like == false)
	{
		if (window.Debug) console.log("dislike message",message.likemsg_id);
	    xmlMsg.up().c("dislike", {xmlns: "urn:xmpp:message-correct", id:message.likemsg_id})
	}
	else if(message.mode == "createnote")
	{
      xmlMsg.up().c("message-type", {xmlns: "urn:xmpp:message-correct:0", value:"NOTE"})
	}
	else if(message.editfilename == true)
	{	
		xmlMsg.up().c("editfilename", {xmlns: "urn:xmpp:message-correct", id:message.editfilename_id,filename:message.msg})
	}
	else if(message.urlLink == true)
	{	
		xmlMsg.up().c("urlLink", {xmlns: "urn:xmpp:message-correct",title:message.urlTitle,summary:message.summary})
   }

	if(message.isTask)
	{
      
   /*   xmlMsg.up().c("message-type", {xmlns: "urn:xmpp:message-correct:0", value:(message.parentid) ? "TEXT": "TASK",  type:"REPLYTASK"}); */

     if(message.parentid)
      xmlMsg.up().c("message-type", {xmlns: "urn:xmpp:message-correct:0", value:(message.parentid) ? "TEXT": "TASK",  type:"REPLYTASK"});
      else
      xmlMsg.up().c("message-type", {xmlns: "urn:xmpp:message-correct:0", value:(message.parentid) ? "TEXT": "TASK"});


      xmlMsg.up().c("thread", {xmlns: "urn:xmpp:message-correct:0", parent:(message.parentid) ? message.parentid : ""}).h(message.taskdetails.UID).up();	   
     // xmlMsg.up().c("thread", {parentid:message.taskdetails.THREAD_ID}).t(message.taskdetails.UID);
	}
	else
	{
		xmlMsg.up().c("thread", {xmlns: "urn:xmpp:message-correct:0", parent:(message.parentid) ? message.parentid : ""}).h((message.parentid) ? generateTaskID() : "").up();
	}
   

   if (window.Debug) console.log("Xml msg final", xmlMsg);
...
jsxc.gui = {
   ...
   queryActions: {
      ...
      message: function(jid, params) {
         ...
         if (params && typeof params.body === 'string') {
            win.find('.jsxc_textinput').html(params.body);
         }
      },
    ...
    */
   init: function() {
	  if (window.Debug) console.log("jsxc.gui_init");
    if ($('#jsxc_windowList').length > 0) {
		  if (window.Debug) console.log("jsxc gui init return");
         return true;
      }
...
//      Tinycon.setBubble(jsxc.storage.getUserItem('unreadMsg') || 0);
	 ...
	 // jsxc.gui.fileShareTemplate = $(jsxc.gui.template.get('filestore'));
   },
   ...
   update: function(bid) {
	   ...
	   if (window.Debug) console.log("Update data is calling..",bid);
      var data = jsxc.storage.getUserItem('buddy', bid);

	   if (window.Debug) console.log("Update data is calling data..",data);
      if (!data) {
	  ...
	  if (window.Debug) console.log("trigger calling ...",bid);
   ...
   showRemoveDialog: function(bid) {
		...
		if (window.Debug) console.log("showRemoveDialog01");
      jsxc.gui.dialog.open(jsxc.gui.template.get('removeDialog', bid));

      var data = jsxc.storage.getUserItem('buddy', bid);

		if (window.Debug) console.log("data01",data);
      $('#jsxc_dialog .jsxc_remove').click(function(ev) {
         ev.stopPropagation();

         if (jsxc.master) {
           
			var jid = jsxc.jidToBid(data.jid);
			jsxc.xmpp.removeBuddy(jid);
			if (window.Debug) console.log("bid123",jid);
			jsxc.storage.removeUserItem('new', jid);
			jsxc.storage.removeUserItem('history', jid);
			jsxc.storage.removeUserItem('member', jid);
			jsxc.storage.removeUserItem('res', jid);
			jsxc.xmpp.removeGroupWindow(jid);
   ...
   showUnknownSender: function(bid, jid) {
	   if (window.Debug) console.log("bid ::: "+bid + "JID ::: "+jid );
   ...
   unreadMsg: function(bid) {
      ...
      if(bid != undefined)
		{
			var split = (bid).split("@")[0];
			var chatWin = split+"-ChatWind";
			var len = $("#ParentWindow").find("."+chatWin).length;
			if( len == 0 )
			{
				var slide = split + "-slide";
				$('.'+slide).find("b").remove();
				if(count != 0)
               $('.'+slide).append('<b class="inmsg">'+count+'</b>');
               total++;
               if(total < 0)
                   total = 0;
			}
...
jsxc.gui.roster = {
    ...
    */
   add: function(bid) {
	   if (window.Debug) console.log("buddyTemplate01");
      var data = jsxc.storage.getUserItem('buddy', bid);
      ...
      $('#jsxc_roster > p').remove();		//jsxc_text
	  ...
	  bud.find('.buddychat').click(function() {
            jsxc.gui.window.open(bid);
      });

	  bud.find('.buddycall_a').click(function() {
              if (window.Debug) console.log("Calling Now:",bid);
              if (window.Debug) console.log("Calling Now:",(bid.split("@")[0]));
			 
            CallNow(bid.split("@")[0],"audio")
      });

      bud.find('.buddycall_v').click(function() {
            //if (window.Debug) console.log("Calling Now:",bid);
            CallNow(bid.split("@")[0],"video")
      });

      bud.find('.buddyscreen').click(function() {
            //if (window.Debug) console.log("Calling Now:",bid);
            CallNow(bid.split("@")[0],"screen")
      });

      bud.find('.buddymeeting').click(function() {
            //if (window.Debug) console.log("Arrange meeting Now with :",bid);
      });
...
jsxc.gui.window = {
   ...
   init: function(bid) {
		...
		var dataext = GetContactDetails(bid.split("@")[0]);
		 ...
		 if (window.Debug) console.log("dataext is",dataext);
		if(data == undefined)
			return false;
		var moderatorstatus = "";
		if(dataext != undefined)
			moderatorstatus = dataext.status_msg || "";
		win.find('.jsxc_status-msg').html(moderatorstatus);
		/*statusApiCallingChat(function(status){
			console.log("status is",status);
			win.find('.jsxc_status-msg').html(status);
		},data.extension);*/
		var imageUrl = 'data:image/png;base64,';
		//if(data.ImageURL)
			//imageUrl = data.ImageURL;

		win.find('.jsxc_avatar').css("background-image", "url(" + imageUrl + ")");
		win.find('#viewproLi').attr('onclick','showUserProfile("'+bid+'")');
		//win.find('.name_fav_icon').css({display: 'block'});
		win.data('jid', bid);	
		checkfavorite(bid);
		checkmutenotice(bid);
		...
		if(contname.length > 23)
			contname = contname.substring(0,23)+"...";
		
		
		win.find(".members_list").append('<li><img src='+globalImg+' title='+loggeduser.username+' class="mr-3 mt-3 rounded-circle" style="height: 35px; width:35px;"></li>');
		if(bid.search("conference") != -1)
		{
			var checkdata = data.res || [];
			
			if(checkdata.length > 0)
			{
				for(var k=0;k<checkdata.length;k++)
				{		//["1660","1659\\40im01.unifiedring.co.uk\\2f307567317914922461624690"]
					var id = checkdata[k];//.split()[0]; 
					if(id != loggeduser.sip_userid){
						var moderate = GetContactDetails(id);
						if(moderate ==  undefined)	continue;
						win.find(".members_list").append('<li><img src='+globalImg+' title='+moderate.caller_id+' class="mr-3 mt-3 rounded-circle" style="width:30px;"></li>');
					}
				}
				win.find(".members_list").append('<li class="elips"><i class="fa fa-ellipsis-h" aria-hidden="true"></i></li>');
				win.find(".members_list").append('<li class="addcontactTeam"><a data-toggle="modal" data-target="#addmemberTeam1"><img src="././images/chat_img/addMember.svg" title="Add Member" class="mr-3 mt-3 rounded-circle" style="width:30px;"></a></li>');
				
				$(".addcontactTeam").click(function()
				{
					//addGroupParticipate(nickname, bid);
				});
			}
		}
		else {
			win.find(".members_list").append('<li><img src='+globalImg+' title='+data.name+' class="mr-3 mt-3 rounded-circle" style="height: 35px; width:35px;"></li>');	
		}
		win.find('.initwindspan').html("Welcome, "+contname);
		$(".slide-toggle").click(function(){
		  
		  if( $(".slide-toggle").hasClass('rotate') )
		  {
			  $(".slide-toggle").removeClass('rotate');
			  $(".slidesection").removeClass('memberwidth');
		  }
		  else{
			  //loadRightSideTask();
			  $(".slide-toggle").addClass('rotate')
	  ...
	  //loadRightSideTask();
		...
		$('.jsxc_textarea').click(function(){
            // $('.jsxc_textinput').focus();
            setCaret();
         });
         win.find('.msettingicon').click(function () {
            if ($('.msettingicon').hasClass('open'))
               $('.msettingicon').removeClass("open");
            else
               $('.msettingicon').addClass("open");

			/*console.log("bid",bid);
			console.log("calling fav",window.LastChatWindow);
            checkfavorite(window.LastChatWindow);
            checkmutenotice(window.LastChatWindow);*/
            checkDesknotice(window.LastChatWindow);
            checkSoundnotice(window.LastChatWindow);
            //checkemailnotice(window.LastChatWindow);
         });
         win.find('.mplusicon').click(function () {
            if ($('.mplusicon').hasClass('open'))
               $('.mplusicon').removeClass("open");
            else
               $('.mplusicon').addClass("open");
			 if (window.Debug) console.log("bid",bid);
			 if (window.Debug) console.log("calling fav",window.LastChatWindow);
            checkfavorite(window.LastChatWindow);
            checkmutenotice(window.LastChatWindow);
            checkDesknotice(window.LastChatWindow);
            checkSoundnotice(window.LastChatWindow);
	...
	win.find('.jsxc_flag').click(function() {
	
		if (window.Debug) console.log("hasClass",$(this).hasClass('setFlagbackground'))
		if( $(this).hasClass('setFlagbackground') )
		{
			isflag = false;
			$(this).removeClass("setFlagbackground");
			$(this).attr("title",'notfill');
		}
		else{
			isflag = true;
			$(this).addClass("setFlagbackground");
			$(this).attr("title",'fill');
		}
	});
	
	win.find('.jsxc_editcancel').click(function() {
	...
	win.find('.jsxc_gif').click(function() {
		gifMainWindow(bid);
	});
	  ...
	  window.bid = bid;
      win.find('.ScreenNow').click(function() {
            CallNow(bid.split("@")[0],"screen")
      });

	  win.find('.BoardNow').click(function () {
          InitBoard(bid);
      });
	  
	  win.find('.Chat_Sec').click(function () {
          if (window.Debug) console.log("Calling Now:", bid);
          	$('#conversation_sec').show();
			$('.whiteboardsec').hide();
			  $('.Chat_Sec').hide();
			$('.BoardNow').show();
      });
      win.find('.VideoNow').click(function() {
            CallNow(bid.split("@")[0],"video")
      });

      win.find('.CallNow').click(function() {
            CallNow(bid.split("@")[0],"audio")
      });
      ...
      win.find('.jsxc_sendFile').click(function() {
		...
		if (window.Debug) console.log("file clicked");
		 $('#userFile').hide();
      ...
      win.find('.jsxc_textinput').keyup(function(ev) {
         ...
         if (ev.which !== 13 || ev.shiftKey || !$(this).html()) {
            resizeTextarea.call(this);
            return;
         }
		if (window.Debug) console.log($(this).html().trim().length);
		if($(this).html().trim().length == 0)
			return;
		if (window.Debug) console.log("after your msg is",$(this).html());
		
		var temp = $('#Parentconversationwindow').hasClass("active");
		if(temp == false)
		{
			$('#userFile').hide();
			$('#conversationwindow').hide();
			...
			$('#ParentuserFile').removeClass("active");
			$('#userFile').removeClass("active in");
			$('#Parentconversationwindow').addClass("active");
			$('#conversationwindow').addClass("active in");
		}
		
		var temp = $('.jsxc_flag').attr("title");
		if((temp == "fill") && (isflag == true))
			isflag	=	 true;
		else
			isflag = false;

		var msg = getText($(this).html());
		var is_edited = false;
		var is_quoted = false;	
		var isedited   = $('.jsxc_textinput').hasClass("editedmsg");
		var isQuotemsg = $('.jsxc_textinput').hasClass("quotemsg");
		var is_editedid = "";
		...
		if(isQuotemsg ==true)
		{
			quotedmsg_id = $("#quotedid").val()
			is_quoted = true;
			msginnerID = quotedmsg_id+"_Inner";
			quotemsg = document.getElementById(msginnerID).innerHTML;
			var Timestampid  = quotedmsg_id+"Timestamp";
			QuoteTimeStamp = document.getElementById(Timestampid).innerHTML;
			//closeCopyQuoteDiv(quotedmsg_id);
		}
		else
		{
			is_quoted = false;
		...
		if(is_edited == true)
		 {
			var exitingid = is_editedid.replace("-",":");
			var data = jsxc.storage.getUserItem('msg', exitingid);
			data.htmlMsg = getText($(this).html());
			data.msg = getText($(this).html());
			data.editedmsg_id = is_editedid;
			data.edited = is_edited;
			jsxc.storage.setUserItem('msg', exitingid, data);
		 }
        var msgdata = jsxc.gui.window.postMessage({
            bid: bid,
            direction: jsxc.Message.OUT,
            msg: msg,
			flag: isflag,
			editedmsg_id : is_editedid,
			edited : is_edited,
			quoted : is_quoted,
			quoted_id : quotedmsg_id,
			quoted_msg : quotemsg,
			quoted_timestamp : QuoteTimeStamp
         });
		//SaveChatRegister(bid, msgdata);
        $(this).css('height', '').html('');
            // $('.jsxc_textinput').focus();
            setCaret();
		isflag = false;
        ...
        ev.preventDefault();
         ...
         var objDiv = document.getElementById("conversation_sec");
         setTimeout(function() {
      ...
      function resizeTextarea() {
         ...
         if ($(this).outerHeight() < (this.scrollHeight - 1) && $(this).html()) {
      ...
      win.find('.jsxc_textarea').click(function() {
        if (typeof getSelection === 'function' && !getSelection().toString()) {
               // $('.jsxc_textinput').focus();
               //  document.getElementsByClassName('jsxc_textinput').focus()
               setCaret();
            }
         });
      ...
      $.each(jsxc.gui.emotions, function(i, val) {
         ...
         li.click(function() {
			...
			$('.jsxc_textinput').append(ins + '&nbsp');
         });
      ...
      $(document).trigger('init.window.jsxc', [win]);	  
	  	if(data.type == "groupchat")
		{					
			$('.rightsidesec').addClass('groupPic')
			$('.rightsidesec').append($(jsxc.gui.template.get('groupprofilepic')));
		}
	 $("#take_selfie_group-photo").click(function() {
		 $("#group_selfie_sec, #group_selfie_save_sec").show();
        var video = document.querySelector('#group-camera-stream'),
            image = document.querySelector('#group-snap'),
            start_camera = document.querySelector('#group-start-camera'),
            controls = document.querySelector('.controls'),
            take_photo_btn = document.querySelector('#group-take-photo'),
            delete_photo_btn = document.querySelector('#group-delete-photo'),
            download_photo_btn = document.querySelector('#group-download-photo'),
            error_message = document.querySelector('#group-error-message');
        ...
        if (!navigator.getMedia) {
            ...
            navigator.getMedia({
                ...
                function(err) {
                    displayErrorMessage("There was an error with accessing the camera stream");
                });
        }
        ...
        take_photo_btn.addEventListener("click", function(e) {
            ...
            loggeduser = JSON.parse( localStorage.getItem('login'));
            loggeduser.profileUrl = snap;
            ...
            var iq = $iq({
                ...
                xmlns: "vcard-temp"
            }).c("PHOTO").c("TYPE").t("image/png").up().c("BINVAL").t(snap.substr(22));

            jsxc.xmpp.conn.sendIQ(iq, function(stanza) {
                $('.jsxc_avatar.sip' + jsxc.bid.split('@')[0]).removeAttr('style');
                jsxc.storage.removeUserItem('avatar', 'own');
                jsxc.storage.removeUserItem('avatar', jsxc.bid);
                var lpresence = window.presence;
                lpresence.c("x", {
                    xmlns: 'vcard-temp:x:update'
                }).c("photo").t(sha1(snap));
                jsxc.xmpp.conn.send(lpresence);
            }, function(stanza) {
	...
	$(document).on("click", "#group_selfie_save", function() {
	$("#group_selfie_sec, #group_selfie_save_sec").hide();	
     var view_rd = "";
	var view_pic = "";
    var xArray = {
        profile_photo_path: $('#grpdb').attr('src'),
        photo_visibility: view_pic,
        user_id: "newgroupchat4@conference.im03.vectone.com"
    };
	 if (window.Debug) console.log("ajax call0")
   ...
   _show: function(bid) {
      ...
      $('[data-bid="' + bid + '"].jsxc_windowItem').find('.jsxc_avatar').css('background',
            $('.jsxc_avatar.sip' + bid.split('@')[0]).css('background'));
   ...
   postMessage: function(message) {
		if( (message.msg == "") || (message.msg == null) || (message.msg == undefined) )     // fix me for null msg stored
		{
		...
		if( (message.like == true) || (message.like == false) || (message.editfilename == true) || (message.bMark == true) || (message.bMark == false) )
		{
		...
		message.bid = 	message.bid.split("/")[0];
      ...
      // exceptions:
		if(!(message.bid).includes("@conference"))
		{
      ...
      try {
        if(!message.edited)
        {
            SaveChatRegister(message.bid, message);
            if(message.direction == "in")
            {
               if(message.mode == "createnote")
               {
                  var msg = message.msg || message.htmlMsg;
                  msg = JSON.parse(msg);
               
                  var temp = [];
                  temp.Id = "";
                  temp.DESCRIPTION = msg.DESCRIPTION;
                  temp.SUMMARY = msg.SUMMARY;
                  temp.UID = msg.UID;
                  temp.DTSTART = msg.DTSTART || new Date().getTime();
                  temp.SENDER = message.bid;
                  temp.MSGID = message._uid;
                  temp.RECEIVER = msg.RECEIVER;
                  temp.GROUPID = msg.GROUPID;
                  temp.OWNERID = msg.OWNERID;
                  if (typeof GnotesDetail === 'undefined') 
                     window.GnotesDetail = [];
               
                  GnotesDetail.push(temp);

                  if($(".mynoteWin").length != 0)
                     loadingnotetable();
               }
               else if(message.isEvent)
               {
                  var msg = message.msg || message.htmlMsg;
                     msg = JSON.parse(msg);
                  if((message.parentId == "") || (message.parentId == undefined))
                  {
                     var temp = [];
                     temp.Id =  '';
                     temp.DURATION = msg.DURATION;
                     temp.LASTMODIFIED = msg.DTSTART;
                     temp.LOCATION = msg.LOCATION;
                     temp.DUETIME = msg.DUETIME;
                     temp.COMPANY_ID = msg.COMPANY_ID;
                     temp.RRULE = msg.RRULE;
                     temp.DTSTART = msg.DTSTART;
                     temp.STATUS = msg.STATUS;
                     temp.RECEIVER = msg.RECEIVER;
                     temp.FMTTYPE = msg.FMTTYPE;
                     temp.MSGID =  message._uid;
                     temp.SENDER = msg.SENDER;
                     temp.TRIGGER = msg.TRIGGER;
                     temp.THREAD_ID = msg.THREAD_ID;
                     temp.OWNERID = msg.OWNERID;
                     temp.CONV_ID = msg.CONV_ID;
                     temp.DESCRIPTION = msg.DESCRIPTION;
                     temp.ASSIGNEECOMPLETED = msg.ASSIGNEECOMPLETED;
                     temp.ACTION = msg.ACTION;
                     temp.DTEND = msg.DTEND;
                     temp.ATTENDEE = msg.ATTENDEE;
                     temp.SUMMARY = msg.SUMMARY;
                     temp.SIPID = msg.SIPID;
                     temp.COMPLETEDWHEN = null;
                     temp.UID = msg.UID;
                     temp.COMPLETEPERCENTAGE = msg.COMPLETEPERCENTAGE;
                     temp.SECTION = msg.SECTION;
                     temp.GROUPID = msg.GROUPID;

                     if(window.GeventDetail == undefined)
                        window.GeventDetail	= [];
                        
                     GeventDetail.push(temp);
                     if($(".eventWin").length != 0)
                        openEntWin();
                  }
                  else{
                     var getTemp = [];
                     getTemp.id = "db-"+(message.bid).split("@")[0]+"_chatHistory";
                     getTemp.msgid = message.parentId; 
                     getTemp.summary = msg.SUMMARY;
                     getTemp.starttime = msg.DTSTART;
                     getTemp.endtime = msg.DTEND;
                     getTemp.location = msg.LOCATION;
                     getTemp.rRule = msg.RRULE;
                     getTemp.description = msg.DESCRIPTION;
                     getTemp.direction = "in";
                     couchDbGetItem(updateEventWin_res, getTemp);
                    //updateEventWin(msg.summary, msg.dtstart, msg.dtend, message.parentId, msg.description, message.bid, '', 'receiver')
                  }
               }
            }
            if(message.quoted)
               updateMentionsDetails(message.bid, message)
            /* if(message.isTask)
               updateTaskDetails(message.bid); */
        }
         else
            {
               if(window.LastChatWindow == message.bid)
               {
                  var chat = (window.LastChatWindow).split("@")[0]+"-ChatWind";
                  var len = $("#ParentWindow").find("."+chat).length;
                  if( len != 0 )
                  {
                     var id = message.editedmsg_id;
                     if(message.editedmsg_id.includes("msg"))
                        id = message.editedmsg_id.replace(":","-");
                     if($("#"+id).length == 1)
                     {
                        $("#"+id).find(".msgTxt").text("");
                        $("#"+id).find(".msgTxt").append('<b><span class="icon-edit weblinks"></span>'+message.msg+'</b>');	
                        //editMessageSave(message.editedmsg_id, message.msg);
                     }
                  }
               }
            }
            ...
            if(message.attachment)
            {
               if( (message.mode == "") || (message.mode == undefined) && (message.mode != 'createnote') )
                   saveFileCouchdb(message._uid, message.bid, new Date(), message.attachment.name, message.attachment.type);
            }
      ...
      if (message.direction === 'in' && !jsxc.gui.window.get(message.bid).find('.jsxc_textinput').is(":focus")) {
		 ...
		 if (window.Debug) console.log("message.attachment :::: "+message.attachment + "message.msg ::: "+message.msg);
		 if(message.attachment == undefined && message.msg == '')
		 {
			//notification fix 
		 }
		else
      ...
      var isTask = message.isTask;

      if(isTask == true)
      {
         var uid 		= 	message._uid;
         var msgID 		= 	uid.replace(/:/g, '-');
         var Taskid 	= 	message.taskdetails.UID;
            if(message.parentid == null || message.parentid == "null" || message.parentid == undefined || message.parentid == "undefined" || message.parentid == "")
            {
               console.log("Task update msg calling");
               updateMSGidforTask(Taskid, msgID, message.taskdetails);  
            }
      ...
      if (message.direction === 'out' && message.msg === '?' && jsxc.options.get('theAnswerToAnything') !== false) {
		  ...
		  if (window.Debug) console.log("Out is calling ....");
   ...
   _postMessage: function(message, restore) {	   
	  ...
	  var msgID = uid.replace(/:/g, '-');
	    uid = msgID.replace("-", ":");
		var len = 0;
		data = jsxc.storage.getUserItem('msg', uid);
		if(data != null)
			len = data.likelist.length;
 
		var msgDiv = $("<div>"),
        msgTsDiv = $("<div>");
	  
		var righclickid= msgID+"_righclick";

         msgDiv.addClass('jsxc_chatmessage jsxc_' + direction);
         msgDiv.addClass('middle_pannel');

         msgDiv.attr('id', msgID);
         msgDiv.attr('oncontextmenu', 'ShowMenu("' + righclickid + '", "' + event + '", "' + msgID + '")');


         var name = "unknow sender";
         if (direction === 'in') {
            var Bdata = jsxc.storage.getUserItem('buddy', bid) || [];
            if (Bdata.length != 0) {
               name = Bdata.name;
            }
         }
	...
	if (message.attachment && message.attachment.name) {
		temp = '';
		msg = message.attachment.name;
	}
	if(message.attachmentmsg){
		temp += " replied to <u>"+ message.attachmentmsg+"</u>";
	}

	if (direction === 'sys') {
		$("#"+msgID+" .rightIconBox").hide();
	}
      ...
      if (!message.forwarded) {	
		...
		{	
         if(window.LastChatWindow == message.bid)
         {
            var chat = (window.LastChatWindow).split("@")[0]+"-ChatWind";
            var len = $("#ParentWindow").find("."+chat).length;
            if( (len != 0 ) && (!message.edited) )
            {
               if($("#"+message._uid.replace(":","-")).length == 0)
               {
                  loadingMsgWin(message, window.LastChatWindow);
                  sentDelivery(bid, message._uid);
			         updateTickOption(bid, message._uid, "particualar");
               }
               
               //loadMessage(window.LastChatWindow);
            }
         }
		}
   },
   ...
   setText: function(bid, text) {
      jsxc.gui.window.get(bid).find('.jsxc_textinput').html(text);
   },
   ...
   setLastMsg: function(bid, msg) {	  
      ...
      lastMsgTextElement.find('a').each(function() {
		  if (window.Debug) console.log("text012345",$(this).text());
		  var lastmsg = $(this).text();
         ...
         $(this).replaceWith('<span>' + lastmsg + '</span>');
      });
   },
   ...
   restoreChat: function(bid) {
	   if (window.Debug) console.log("Restore chat :::", bid);
      var chat = jsxc.storage.getUserItem('chat', bid);

	  if (window.Debug) console.log("chat calling", chat);	
	  ...
	  if (window.Debug) console.log("history calling", history);
   ...
   updateProgress: function(message, sent, size) {
      ...
      var perc = Math.round(sent / size * 100) + '%';
      span.text(' ' + perc);
   ...
   showOverlay: function(bid, content, allowClose) {
	  ...
	  if (window.Debug) console.log("win_append",win);
   },
...
// async function createFile(path,mimeType,fileName){
// 	if (window.Debug) console.log("file232");
...
//   if (window.Debug) console.log("file222",file);
...
{
	if (window.Debug) console.log('---getText---',text);
	for (var key in emoji) {
		if (emoji.hasOwnProperty(key)) {
			if (window.Debug) console.log("key",key);
			if (window.Debug) console.log("emoji[key]",emoji[key]);
			text = replaceAll(text, key, emoji[key]);
		}
	}
	if (window.Debug) console.log("text is",text);
	return text;
}
...
jsxc.fileTransfer.showFileSelection = function(jid) {
    if (window.Debug) console.log("jid is",jid);
   var bid = jsxc.jidToBid(jid);
   var msg = $('<div class="thirdfileshare"><ul><li class="btn-bs-file btn btn-xs btn-primary"><label>Browse local<input type="file" name="files" id="fileElem" multiple accept="image/*" /><label></li><li><label id ="cloudStorageGoogle" class="btn-bs-file btn btn-xs btn-primary">Google Drive</li><li><label id ="cloudStorageMicrosoft" class="btn-bs-file btn btn-xs btn-primary">OneDrive</li><li><label id ="cloudStorageDropBox" class="btn-bs-file btn btn-xs btn-primary">Dropbox</li><li><label id ="cloudStorageBox" class="btn-bs-file btn btn-xs btn-primary">Box</li><li><label id ="cloudStorageevernote" class="btn-bs-file btn btn-xs btn-primary">Evernote</li><li></ul></div>');
	
	//var msg = $('<div class="thirdfileshare"><ul><li><label class="btn-bs-file btn btn-xs btn-primary">Browse local<input type="file" name="files" id="fileElem" multiple accept="image/*" /><label></li><li><label id ="cloudStorageGoogle" class="btn-bs-file btn btn-xs btn-primary">Google Drive</li><li><label id ="cloudStorageMicrosoft" class="btn-bs-file btn btn-xs btn-primary">OneDrive</li></ul></div>');
		
	msg.addClass('jsxc_chatmessage');
	jsxc.gui.window.showOverlay(bid, msg, true);
	//msg.addClass('jsxc_chatmessage');
	//$(".jsxc_chatmessage").attr('id',Math.random());
 
   // open file selection for user
  // msg.find('label').click();

	msg.find('#cloudStorageGoogle').click(function(){
	...
	msg.find('#cloudStorageMicrosoft').click(function(){
		ipcRenderer.send('Microsoftfiles', 'ping');
		jsxc.gui.window.hideOverlay(bid);
		
		if( $('.cloudParent').length == 0 )
				$( ".jsxc_fade" ).prepend("<div class=cloudParent><div class=cloudfile bid="+bid+"></div></div>" );
	});
	msg.find('#cloudStorageDropBox').click(function(){
		ipcRenderer.send('DropBoxfiles', 'ping');
		jsxc.gui.window.hideOverlay(bid);
		if( $('.cloudParent').length == 0 )
				$( ".jsxc_fade" ).prepend("<div class=cloudParent><div class=cloudfile bid="+bid+"></div></div>" );
	});

	msg.find('#cloudStorageBox').click(function(){
		ipcRenderer.send('Boxfiles', 'ping');
		jsxc.gui.window.hideOverlay(bid);
	});
	
	msg.find('#cloudStorageevernote').click(function(){
		ipcRenderer.send('EverNotefiles', 'ping');
		jsxc.gui.window.hideOverlay(bid);
	});
	
   msg.find('[type="file"]').change(function(files) {
		...
		for (var i =0;i<size;i++)
		{
			//msg.addClass('jsxc_chatmessage');
			//jsxc.gui.window.showOverlay(bid, msg, true);
			//if (window.Debug) console.log("i value",i);
			img = "";
		...
		$('.fileSend').click(function(){
			...
			for (var i =0;i<size;i++)
			{
				...
				var imgSrc = $(".img[title|='"+file.name+"']").attr('src');		
				 if (window.Debug) console.log("file is",file);
				...
				var message = jsxc.gui.window.postMessage({
				 ...
				 if (window.Debug) console.log("message is",message);
...
{
	if (window.Debug) console.log("dropfile123",dropfile);
}


 jsxc.fileTransfer.fileSelected = function(jid, msg, file,img) {
   window.waste = file;
   var bid = jsxc.jidToBid(jid);
   ...
   if (file.transportMethod !== 'webrtc' && jsxc.xmpp.httpUpload.ready && maxSize >= 0 && file.size > maxSize) {
	   if (window.Debug) console.log("File too large for http upload. 01");
      jsxc.debug('File too large for http upload.');

      if (jsxc.fileTransfer.isWebrtcCapable(bid)) {
         // try data channels
         file.transportMethod = 'webrtc';
		 if (window.Debug) console.log(" file.transportMethod is webrtc");
		 ...
		 if (window.Debug) console.log("showFileTooLarge");
         jsxc.fileTransfer.showFileTooLarge(bid, file);
      }
      return;
   } else if (!jsxc.xmpp.httpUpload.ready && Strophe.getResourceFromJid(jid)) {
      // http upload not available
	   if (window.Debug) console.log(" file.transportMethod is webrtc 02");
      file.transportMethod = 'webrtc';
   }
   ...
   if (window.Debug) console.log("check type01",file.type);
   if(file.type == "gif") {
	    attachment.addClass('jsxc_image-jpeg');
		attachment.addClass('jsxc_image ');
   }
   else 
   {
   ...
   if (FileReader && file.type.match(/^image\//)) {
      // show image preview
		 ...
		 // var button = "<button id='abortbtn' class='abortbtn' fname="+file_name+">abort</button>";
	...
	/*if (window.Debug) console.log("near ok button");
   $('<button>').addClass('jsxc_btn jsxc_btn-primary').text($.t('Send')).click(function() {
      ...
      if (file.transportMethod === 'webrtc') {
         ...
         sess.sender.on('progress', function(sent, size) {
            ...
            if (sent === size) {				
               message.received();
			   if (window.Debug) console.log("return1");
			   return;
            }
         });
      } else {
...
jsxc.fileTransfer.updateIcons = function(bid) {
	 if (window.Debug) console.log("updateIcons ::: ", bid)
   var win = jsxc.gui.window.get(bid);
if (window.Debug) console.log("updateIcons win ", win);
  
   if (!win || win.length === 0 || !jsxc.xmpp.conn) {
      return;
   }

   if (window.Debug) console.log("After updateIcons win ", win);
   jsxc.debug('Update file transfer icons for ' + bid);

   if (jsxc.xmpp.httpUpload.ready) {
	   if (window.Debug) console.log("jsxc.xmpp.httpUpload.ready ");
   ...
   if (window.Debug) console.log("After jsxc.xmpp.httpUpload.ready ");
   var jid = win.data('jid');
   var res = Strophe.getResourceFromJid(jid);
   var fileCapableRes = jsxc.webrtc.getCapableRes(bid, jsxc.webrtc.reqFileFeatures);
   var resources = Object.keys(jsxc.storage.getUserItem('res', bid) || {}) || [];

   if (window.Debug) console.log("fileCapableRes ", fileCapableRes);
...
$(document).on('update.gui.jsxc', function(ev, bid) {
	...
	if (window.Debug) console.log("update qui calling ;;; ", bid)
   ...
   if (avatarSrc !== null) {
      ...
      var handler_cb = function(stanza) {
         var src = jsxc.gui.avatar.getPhotoFromVcard(stanza);
            ...
            self.set(jid, el, src);
            if((src !=0) && (jid != jsxc.bid))
               updateUserImg(jid, src);
      };
...
//   if (src.startsWith('data:image/png;base64,data')) src = src.substr(22);
   //if (window.Debug) console.log(src);
   if (jid == jsxc.bid) {
//         Send Presence Again
      var lpresence = window.presence;
      lpresence.c("x", {
      ...
      jsxc.xmpp.conn.send(lpresence);
//         //if (window.Debug) console.log('Send presence', window.presence.toString(),  sha1(src) );
}
setTimeout(function(){
   $('.jsxc_avatar.sip' + jid.split('@')[0]).removeAttr('style').css({
      'background-image': 'url(' + src + ')',
      'text-indent': '999px'
   });

   $('[data-bid="' + jid + '"].jsxc_windowItem').find('.jsxc_avatar').css('background',
   $('.jsxc_avatar.sip' + jid.split('@')[0]).css('background'));
...
jsxc.Message.prototype.save = function() {
   ...
   if (this.bid) {
	 ...
	 // if(this.edited == false)
      ...
      if (history.indexOf(this._uid) < 0 && (this.edited == false || this.edited == null || this.edited == undefined || this.edited == "")) {
   ...
   if (window.Debug) console.log(this.like)
   if(this.like == undefined) {
	   if (window.Debug) console.log("like msg saving");
	   this.likelist = [];
	   jsxc.storage.setUserItem('msg', this._uid, this);

	   if (history) {
		 //history.unshift(this._uid);
			history.push(this._uid);
...
jsxc.Message.prototype.received = function() {
	if(window.Debug)	console.log("status for msgread");
	this._received = true;
	this.save();
	updatemessage(this,"readStatus");
   var timstampid = this._uid.replace(/:/g, '-');
	$('#'+timstampid).find(".icon-message-read").remove();
	$('#'+timstampid).find("h6").append('<span class="icon-message-read kane"></span>');
 };
...
if(window.Debug)	console.log("delete function calling...");
   /*var data = jsxc.storage.getUserItem('msg', uid);
   if (data) {
...
jsxc.muc = {
    ...
    */
    showJoinChat: function(r, p) {
		var self = jsxc.muc;
      var room = r+"@"+window.ConferenceDomain;
      if(id == undefined)  return;
		getGroupName(id, function(data)
		{
         if((data != "") && (data != undefined))
         {
            //var nickname = (jsxc.xmpp.conn.jid).split("@")[0];
            self.join(room, jsxc.xmpp.conn.jid, null, data, undefined, true, true);
            showNotification('Team Invite',data);
            //var bl = jsxc.storage.getUserItem('buddylist');
            //if (bl.indexOf(room) < 0) {
               //bl.push(room); 
               SaveChatRegister(room);	
               //getTeamDetail(room, data.roomname);
            //}
            //loadContact();
            loadTeamContact();
	...
	addGroupParticipate: function(nickname,room) {
		if(nickname != "")
		{
			if(jsxc.bid == nickname)
			return;
		}
		var fromid = jsxc.xmpp.conn.jid;	
		/*var z = $msg({
			to: nickname, from: fromid
		}).c('x', {
			xmlns: 'jabber:x:conference', 
			jid: room+"@conference.im01.unifiedring.co.uk"
		});*/
		var z = $msg({
			to: nickname, from: room+"@"+window.ConferenceDomain
		}).c('x', {
			xmlns: 'jabber:x:conference', 
			jid: room+"@"+window.ConferenceDomain
		});
   ...
   /*showJoinChat: function(r, p) {
	...
	if (window.Debug) console.log("Show join Chant Calling....");

	
		 //jsxc.gui.window.clear(r);
         //jsxc.storage.setUserItem('member', r, {});
         //self.join(r, "", p, "", "", false, false);
           //GetBuddies(); 
	...
	if (window.Debug) console.log("After Show join Chant Calling....");	
         ...
         if (jsxc.xmpp.conn.muc.roomNames.indexOf(room) < 0) {
            ...
            var discoReceived = function(roomName, subject) {
               ...
               dialog.find('.jsxc_join').click(function(ev) {
                  ...
                  self.join(room, nickname, password, roomName, subject, true, true);
                  GetBuddies();
                  return false;
               });
            };
   ...
   _showRoomConfiguration: function(room, config) {
      ...
      form.submit(function(ev) {
         ...
         self.conn.muc.saveConfiguration(room, config, function() {
			...
			if (window.Debug) console.log("Before Submit list", bl)
            if (bl.indexOf(room) < 0) {
   ...
   initWindow: function(event, win) {
      ...
      if (roomdata == null || roomdata.type !== 'groupchat') {
         return;
      }
      ...
      var ml = $('<div class="jsxc_memberlist" style="display:none"><ul></ul></div>');
     ...
     /* var toggleMl = function(ev) {
      ...
      mlIcon.click(toggleMl);*/
	   mlIcon.click(function() { 
			//$('.jsxc_memberlist').css("display","block");
			  $(".jsxc_memberlist").toggle();
			 loadcontactforAddParty();
	   });
     ...
     //win.find('.jsxc_caption').css("float", "none");
     win.find('.editgroupnamebtn').show();
     
     
     win.find('.editgroupnamebtn').click(function() { 
      
      win.find('.editgroupnamesec').show();
      win.find('.editgroupnamebtn').hide(); 
      win.find('.jsxc_name').hide(); 
      
      win.find('.editgroupnameinput').val( win.find('.jsxc_name').first().text());

   });

   win.find('.saverenamebtn').click(function() { 
      
      win.find('.editgroupnamesec').hide();
      win.find('.editgroupnameinput').val();
      win.find('.editgroupnamebtn').show();
      win.find('.jsxc_name').show();
     var newName =  win.find('.editgroupnameinput').val();
     jsxc.gui.roster._rename(bid, newName);
      

   });

   win.find('.cancelrenamebtn').click(function() { 
      ...
      if (roomdata.state > self.CONST.ROOMSTATE.INIT) {
		 ...
		 var addParticipant={jid:"addParticipant",role:"addParticipant"};
		 self.insertMember(bid, "", addParticipant);
      }
   ...
   onPresence: function(event, from, status, presence) {
      ...
      if(window.Debug)
      {
         ...
         console.log("presence05",presence);
      }

		var show = $(presence).find('show').text();
      var statusIcn 	=	from.split("@")[0] + "-status";
      var pref = $(presence).find('status').text();
      var splt = from.split("@")[0];
		var index = -1;
      if(!from.includes('@conference'))
      {
         index = contacsarray.findIndex(function(item){ (item.sip_login_id == splt)})
      }
      if(show != "")
      {
         switch(show)
         {
            case "chat":
               $("."+statusIcn).attr("src","images/green-active.svg");
               $("."+statusIcn).css("visibility","unset");
               if(index != -1)
                  contacsarray[index].user_status = "Available"; 
               break; 
            case "away":
               $("."+statusIcn).attr("src","images/yellow.png");
               $("."+statusIcn).css("visibility","unset");
               if(index != -1)
                  contacsarray[index].user_status = "Invisible"; 
               break;
            case "dnd":
               $("."+statusIcn).attr("src","images/red.png");
               $("."+statusIcn).css("visibility","unset");
               if(index != -1)
                  contacsarray[index].user_status = "Do not disturb";
               break;
            case "xa":
               $("."+statusIcn).attr("src","images/grey.png");
               $("."+statusIcn).css("visibility","unset");
               if(index != -1)
                  contacsarray[index].user_status = "Busy";
               break;
            default:
               $("."+statusIcn).attr("src","");
              // $("."+statusIcn).css("visibility","unset");
               if(index != -1)
                  contacsarray[index].user_status = "UnAvailable"; 
               break;
         }
      }
      ...
      if(pref != "")
      {
         contacsarray[index].status_msg = pref;
         var chat = (from).split("@")[0]+"-ChatWind";
         var len = $("#ParentWindow").find("."+chat).length;
         if(len != 0 ) 
         {
            var img ="";
            ...
            {
               case "In a meeting":
                  img 		=	"images/in_meeting.svg";
                  break;
               case "Out sick":
                  img 		=	"images/out-sick.svg";
                  break;
               case "Working remotely":
                  img 		=	"images/working_remotely.svg";
                  break;
               case "On holiday":
                  img 		=	"images/on_holiday.svg";
                  break;
               case "Travelling":
                  img 		=	"images/travelling.svg";
                  break;
               case "Out office":
                  img 		=	"images/out_office.svg";
                  break;
            }
            $("#ParentWindow").find('.ministatus').empty().append('<img src="images/out_office.svg">' + "  &nbsp " + pref);
         }
         else if( $('.profileWinM').length !=0 )
         {
            if(contacsarray[index].ext == $('.profileWinM').find("#proExt").html())
            {
               var str = contacsarray[index].caller_id + " - " + pref;
               $(".profileName").empty().html(str);
            }
         }
      }
   ...
   insertMember: function(room, nickname, memberdata) {
      ...
      if(nickname != "")
      {
		  if(jsxc.bid == nickname+"@"+window.ChatDomin)
			return;
      }
	  
	 // var nickname_data = nickname.split("/");
	  
	 // nickname = nickname_data[0];
      ...
      if (m.length === 0) {
		...
		if (window.Debug) console.log("jid",jid);
	
		
		var addparticipabtHTML =
		'<div id="participantInnerDiv" class="modal fade" role="dialog">\n'+
		'<div class="modal-dialog">\n'+
		'<div class="modal-content">\n'+
		'<div class="modal-header">\n'+
        '<button type="button" class="close" data-dismiss="modal" id="invitepopupclose">&times;</button>\n'+
        '<h4 class="modal-title">Modal Header</h4>\n'+
		'</div>\n'+
		'<div class="modal-body">\n'+
		'<div class="groupAddBuddy" id="invitebuddysec"></div>\n'+
		'</div>\n'+
		'</div>\n'+
		'</div>\n'+
		'</div>\n'+
		...
		if(jid == "addParticipant")
			m = $('<li ><a href="#" class="jsxc_avatar" id="addParticipant" data-toggle="participantInnerDiv" data-target="#participantInnerDiv" onclick="loadcontactforAddParty()">+</a> <div class="jsxc_name"/>'+addparticipabtHTML+'</li>');
		else
			m = $('<li><div class="jsxc_avatar user_part"></div><div class="jsxc_name"/></li>');
		...
		m.find('#addParticipant').click(function() {
			
			$('#participantInnerDiv').addClass("in");
			$('#participantInnerDiv').css("display","block");

			var member = jsxc.storage.getUserItem('member', room) || {};
			$.each(member, function(nickname, val) {
							if (window.Debug) console.log("val is",val);
			})


			$('.close').click(function (){
				$('#participantInnerDiv').removeClass("in"); 
				$('#participantInnerDiv').css("display","none"); 
			});
			
			
			
	
			/*Buddies = jsxc.storage.getUserItem('buddylist');
			if (Buddies != null){
				if (window.Debug) console.log("overall Buddies",Buddies);
				//["2171079@im03.vectone.com", "2171086@im03.vectone.com", "p.aricovindane___mundio.com@im03.vectone.com", "2171055@im03.vectone.com", "newgroupchat@conference.im03.vectone.com"]
				
				var member = jsxc.storage.getUserItem('member', room) || {};
				if (window.Debug) console.log("overall participant in grouop",member);
				for(var i=0;i<Buddies.length;i++)
				{
					var temp = false;
					var checkConf = (Buddies[i].split("@")[1]).split(".")[0];
					if(checkConf != "conference") {
						if (window.Debug) console.log("Buddies",Buddies[i]);	
						$.each(member, function(nickname, val) {
							if (window.Debug) console.log("val is",val);
							jid = val.jid;
							if(jid != null)  {
					...
					if(temp)
						if (window.Debug) console.log("Buddies[i]",Buddies[i]);
				}
				if (window.Debug) console.log("end001");
				...
				if (room != undefined )
					window.LastChatWindow = room;
				else
					return;
				
				var z = $msg({
					to: "2171079@im03.vectone.com", from: jsxc.bid
				}).c('x', {
					xmlns: 'jabber:x:conference', 
					jid: window.LastChatWindow
				});
				window.invitation.push(z)
				if (window.Debug) console.log(z)
			...
			//<message to='2171079@im03.vectone.com' from='2171078@im03.vectone.com/10675345898843836864263764' xmlns='jabber:client'><x xmlns='jabber:x:conference' jid='bala@conference.im03.vectone.com'/></message>				
			}*/
		});
   ...
   onGroupchatMessage: function(message) {
      ...
      if (id && jsxc.el_exists(jsxc.Message.getDOM(id)) && message.getElementsByTagName('like').length == 0 && message.getElementsByTagName('unlike').length == 0) {
		...
		var isTask = false;
		var isFile = false;
      if ( (message.getElementsByTagName('like').length > 0))
      {
         var fromUser = ($(message).attr('from')).split("/")[0];
         var isfromUserdata = ($(message).attr('from')).split("/")[1];
         var isFromUser = isfromUserdata.split("\\40")[0]
         var msgID = message.getElementsByTagName('like')[0].getAttribute('id');
         if(isFromUser != jsxc.bid.split("@")[0])
            likemessage(msgID, fromUser, 'like', 'in', fromUser);

         return true;
      }

      if ( (message.getElementsByTagName('unlike').length > 0))
      {
         var fromUser = ($(message).attr('from')).split("/")[0];
         var msgID = message.getElementsByTagName('unlike')[0].getAttribute('id');
         var isfromUserdata = ($(message).attr('from')).split("/")[1];
         var isFromUser = isfromUserdata.split("\\40")[0];
         if(isFromUser != jsxc.bid.split("@")[0])
            likemessage(msgID, fromUser, 'unlike', 'in', fromUser);
      ...
      if ( (message.getElementsByTagName('pin').length > 0))
      {
         var fromUser = ($(message).attr('from')).split("/")[0];
         var msgID = message.getElementsByTagName('pin')[0].getAttribute('id');
         
         setFlagMode(msgID, true, fromUser,'received');
         return true;
      }
      if ( (message.getElementsByTagName('unpin').length > 0))
      {
         var fromUser = ($(message).attr('from')).split("/")[0];
         var msgID = message.getElementsByTagName('unpin')[0].getAttribute('id');
         
         setFlagMode(msgID, false, fromUser,'received');
         return true;
      }
      if ( (message.getElementsByTagName('displayed').length > 0))
      {
			var id = message.getElementsByTagName('displayed')[0].getAttribute('id');
			var fromUser = ($(message).attr('from')).split("/")[0];
        /* var timstampid = id.replace(/:/g, '-');
         $('#'+timstampid).find(".icon-message-read").remove();
         $('#'+timstampid).find("h6").append('<span class="icon-message-read"></span>');*/
         /*setTimeout(function()
         {
            
         },1000)*/
         //getMesssageTick(id, fromUser);
         getMesssageTick(id, fromUser, function(data)
         {
            console.log("return");
         })
      ...
      if (message.getElementsByTagName('message-type').length > 0) {
         var check = message.getElementsByTagName('message-type')[0].getAttribute('value');
         var tagType = message.getElementsByTagName('message-type')[0].getAttribute('type');
         if(check == "NOTE")
         {
            checkNote = 'createnote';
            if (message.getElementsByTagName('replace').length > 0) { 
               try{
                  var notemsgdata = $(message).find('body:first').text();
                  var fromUser = ($(message).attr('from')).split("/")[0];
                  var Msgid = message.getElementsByTagName('replace')[0].getAttribute('id');
                  notemsgdata = JSON.parse(notemsgdata);
                  getNoteUpdate(Msgid, fromUser, "receiver", notemsgdata.UID, notemsgdata.SUMMARY, notemsgdata.DESCRIPTION, notemsgdata.DTSTART, function(data)
                  {
                     if(window.Debug)	console.log(data);
         ...
         {
            var Taskdetailsdata = message.getElementsByTagName('body')[0].childNodes[0].nodeValue;
          
   
            if (message.getElementsByTagName('replace').length == 0)
            Parentid = message.getElementsByTagName('thread')[0].getAttribute('parent');
      ...
      if (message.getElementsByTagName('deleted').length > 0) {
         var fromUser = ($(message).attr('from')).split("/")[0];
         var mid = message.getElementsByTagName('deleted')[0].getAttribute('id');
         checkDeleteMsg(mid, fromUser);
      }

      if ( (message.getElementsByTagName('fileedit').length > 0))
      {
         var id = message.getElementsByTagName('fileedit')[0].getAttribute('id');
         var filename = message.getElementsByTagName('fileedit')[0].getAttribute('filename');
         var fromUser = ($(message).attr('from')).split("/")[0];
         editFileFameCallB(id, filename, fromUser, function(data)
         {
            //console.log("success");
      ...
      if (message.getElementsByTagName('copyquote').length > 0) {
         if (window.Debug) console.log("Quote is calling");
         var xmlns = message.getElementsByTagName('copyquote')[0].getAttribute('xmlns');
         var uid = message.getElementsByTagName('copyquote')[0].getAttribute('id');
         var msg = message.getElementsByTagName('copyquote')[0].childNodes[0].nodeValue;
         var timestamp = message.getElementsByTagName('copyquote')[0].getAttribute('quotedtime');
         mentionid   =   message.getElementsByTagName('copyquote')[0].getAttribute('mentionid');
         var id = uid.replace(":", "-");
      ...
      if (message.getElementsByTagName('edit').length > 0) {
			var xmlns = message.getElementsByTagName('edit')[0].getAttribute('xmlns');
			var uid = message.getElementsByTagName('edit')[0].getAttribute('id');
			var msg = message.getElementsByTagName('body')[0].childNodes[0].nodeValue;
			var fromUser = ($(message).attr('from')).split("/")[0];
			var id = uid.replace(":","-");
			$("#"+id).find(".msgTxt").text("");
			$("#"+id).find(".msgTxt").append('<b><span class="icon-edit weblinks"></span>'+msg+'</b>');	
      ...
      var summary		=	"";
		...
		if(message.getElementsByTagName('urlLink').length > 0) {
			urlTitle	   = 	message.getElementsByTagName('urlLink')[0].getAttribute('title');
			summary		= 	message.getElementsByTagName('urlLink')[0].getAttribute('summary');
			urlLink		=	true;
      }
      if (message.getElementsByTagName('message-type').length > 0) {
         var check =message.getElementsByTagName('message-type')[0].getAttribute('value');
      ...
      if (body !== '') {
            ...
            /* var isFile = $(message).find('body[xmlns="' + Strophe.NS.XHTML + '"]').find('>a').text().match(/^([a-z]+\/[a-z0-9-+.*]+)\|(\d+)\|([\s\w.,-]+)/);
            ...
            if (isFile) {
               ...
               attachment.data = $(message).find('body[xmlns="' + Strophe.NS.XHTML + '"]').find('>a').attr('href')

               body = $(message).find('body[xmlns="' + Strophe.NS.XHTML + '"]').find('>a').attr('href');
...
jsxc.notice = {
   ...
   add: function(data, fnName, fnParams, id) {
         if (window.Debug) console.log("Add Notification: ",data.msg);
      var nid = id || Date.now();
      ...
      if (!id) {
		 ...
		 //var count = description.indexOf('You received a message from an unknown sender');
		//if(count != 0) {
...
jsxc.notification = {
   ...
   init: function() {
      $(document).on('postmessagein.jsxc', function(event, bid, msg) {
		...
		//var sounddata = jsxc.storage.getUserItem('soundsetting', bid);
		
		var sounddata = jsxc.storage.getUserItem('buddy', bid) || {};
		sounddata = sounddata.msgsound || 'Default';
		
		var soundFilename = jsxc.CONST.SOUNDS.MSG;
		switch(sounddata)
		{
			case 'Default':
				soundFilename = "default_msg.mp3";
				break;
			case 'Ping':
				soundFilename = "Ping1.mp3";
				break;
			case 'Air':
				soundFilename = "incomingMessage.mp3";
				break;
		}
		...
		//console.log('soundFilename ', soundFilename);
         ...
         if(data != undefined)
         {
            if(data.is_muted != 1)
            {
               ...
               var userslist = jsxc.storage.getUserItem('desktopnotifyusers') || [];
               if(userslist.indexOf(bid) != -1)
               {
                  jsxc.notification.playSound(soundFilename, undefined, undefined);
               }
            }
         }
      });
   ...
   notify: function(title, msg, d, force, soundFile, loop, source) {
      ...
      var icon = '../favicon.ico' || o.icon || jsxc.options.root;
   ...
   if (window.Debug) console.log("prepareRequest ...");
...
jsxc.otr = {
   ...
   sendMessage: function(jid, msg, message) {
	   if( jsxc.otr.objects[jsxc.jidToBid(jid)] )
	   {
   ...
   create: function(bid) {
      ...
      jsxc.otr.objects[bid].on('io', function(msg, message) {
         ...
         if(jid == undefined && message.isTask == true)
             jid = message.bid;
   ...
   createDSA: function() {
      ...
      if (jsxc.storage.getUserItem('key') === null) {
         ...
         if (!jsxc.otr.dsaFallback) {
            ...
            worker.postMessage({
               imports: [
                  jsxc.options.root + '/../../vendor/salsa20.js', 
                  jsxc.options.root + '/../../vendor/bigint.js', 
                  jsxc.options.root + '/../../vendor/crypto.js', 
                  jsxc.options.root + '/../../vendor/eventemitter.js', 
                  jsxc.options.root + '/../const.js', 
                  jsxc.options.root + '/../helpers.js', 
                  jsxc.options.root + '/../dsa.js'],
               seed: BigInt.getSeed(),
               debug: true
            });
...
jsxc.storage = {
   ...
   getUserItem: function(type, key) {
	  ...
	  //if (window.Debug) console.log("key for getuseritem",key);
         ...
         while (history.length > 0) {
            ...
            el = message.getDOM();
               ...
               if (el.length === 0) {
                  ...
                  if (window.Debug) console.log("On storage calling...");
                  jsxc.gui.window._postMessage(message, true);
               } else if (message.isReceived()) {
                  //el.addClass('jsxc_received');

                  var timstampid = el._uid.replace(/:/g, '-');
                  var stampid = timstampid + "Timestamp";
                  $("#" + stampid).addClass('jsxc_received');
               }
            }
            return;
         }
...
//      Tinycon.setBubble(parseInt(e.newValue) || 0);
...
jsxc.webrtc = {
   ...
   getTurnCrendentials: function(url) {
      ...
      if (ttl > 0) {
		 ...
		 if (window.Debug) console.log("ajax call1")
      $.ajax(url, {
         async: true,
         xhrFields: {
   ...
   getCapableRes: function(jid, features) {
	  ...
	  if (window.Debug) console.log("features ", features);
	  if (window.Debug) console.log("features ", res);
      if (!features) {
         return res;
      ...
      $.each(res, function(i, r) {
	    ...
	    if (window.Debug) console.log("available ", available);
      return available;
   },
   ...
   onPresence: function(ev, jid, status, presence) {
      ...
      if(window.Debug)
         console.log("presence1",presence);
...
jsxc.xmpp.chatState.onComposing = function(ev, jid) {
   ...
   console.log("win composing",win);
   console.log("msg composing",msg);
...
jsxc.xmpp.chatState.startComposing = function(bid) {
   ...
   var type = (bid.includes("conference")) ? 'groupchat' : 'chat';
...
jsxc.xmpp.httpUpload.sendFile = function(file, message) {
	 if (window.Debug) console.log("httpUpload.sendFile",file);
   ...
   self.requestSlot(file, function(data) {
      if (!data) {
         ...
         $('#referenceTxt').find('.progress').css("display","none");
      } else if (data.error) {
         // specific error
		...
		if (window.Debug) console.log("slot received, start upload to");
...
jsxc.xmpp.httpUpload.uploadFile = function(url, file, message, success_cb) {
   ...
   $('#referenceTxt').find('.progress').css("display","block");
   ...
   $.ajax({
      ...
      data: file,
	 // headers: { "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "GET, POST, PUT, OPTIONS, DELETE", 'Access-Control-Allow-Headers':'Content-Type, Origin, X-Requested-With"' },
      processData: false,
      xhr: function() {
      ...
      success: function() {
      	if (window.Debug) console.log("file successful uploaded",message);
        jsxc.debug('file successful uploaded');
	   //SharedFileRegister('OUT', message.bid ,message.attachment, message.stamp, message._uid);
         // In case that upload progress is not available, inform user
         jsxc.gui.window.updateProgress(message, 1, 1);
         getthumbnail(url, message);
         if (success_cb) {
            success_cb();
         }
      },
      error: function(e) {
         $('.loadersimg').css("display","none");
         $('#referenceTxt').find('.progress').css("display","none");
		 if (window.Debug) console.log("error is",e);
...
jsxc.xmpp.mam.nextMessages = function(bid) {
   ...
   if(window.Debug)	 console.log("query end");
};
...
var gifImageHtml =
'<div id="gifshow" class="modal fade" role="dialog">\n'+
'<div class="modal-dialog">\n'+
'<div class="modal-content">\n'+
'<div class="modal-header">\n'+
'<h4 class="modal-title">Powered By <b>GIPHY</b></h4>\n'+
'<button type="button" class="close" data-dismiss="modal">&times;</button>\n'+
'</div>\n'+
'<div class="modal-body">\n'+
'<div class="gifinfo"></div>\n'+
'</div>\n'+
'</div>\n'+
'</div>\n'+
'</div>\n'+
...
var viewPro =
'<div id="userviewProfile" class="modal fade" role="dialog">\n'+
'<div class="modal-dialog">\n'+
'<div class="modal-content">\n'+
'<div class="modal-header">\n'+
'<button type="button" class="close" data-dismiss="modal">&times;</button>\n'+
...
var KeyboardInfo ='<div id="keyboardinfo" class="modal fade" role="dialog">\n'+
'<div class="modal-dialog">\n'+
'<div class="modal-content">\n'+
'<div class="modal-header">\n'+
'<button type="button" class="close" data-dismiss="modal">&times;</button>\n'+
...
'';
 ...
 $('.dropdown-submenu a.downtest').on("click", function(e){
    $(this).next('ul').toggle();
    e.stopPropagation();
    e.preventDefault();
  });
...
jsxc.gui.template['chatWindow'] = '<li class="jsxc_windowItem">\n' +
      '   <div class="jsxc_window chatwindow_sec">\n' +

      '      <div class="jsxc_bar">\n' +
      '         <div class="jsxc_avatar  rightsidesec"></div>\n' +
      '         <div class="jsxc_tools">\n' +

      '</div>\n' +

      '         <div class="jsxc_caption">\n' +
      '            <div class="jsxc_caption_container">\n' +
      '<div class="input-group editgroupnamesec" style="width: 300px; top: 21px; display:none">\n' +
      '<input class="form-control editgroupnameinput" aria-label="Text input with multiple buttons"> \n' +
      '<div class="input-group-btn"> <button type="button" class="btn btn-default saverenamebtn" aria-label="Help"><span class="glyphicon glyphicon-floppy-saved"></span></button> <button type="button" class="btn btn-default cancelrenamebtn"><span class="glyphicon glyphicon-floppy-remove"></span></button> </div>\n' +
      '</div>\n' +
      '<button type="button" class="btn btn-default editgroupnamebtn" aria-label="Help" style="position: absolute; bottom: 51px; left: 27%; display:none"><span class=" glyphicon glyphicon-edit"></span></button>\n' +
      
      '<div class="flex-container-start">\n'+
      '<div class="flex-box">\n'+
      '            	<div class="jsxc_name" />\n' +
      
      '<span class="name_fav_icon" style="display:none"><i class="fa fa-star gold" aria-hidden="true"></i></span>'+
      '<span class="icon-voice-2 name_muteall_icon" style="display:none"></span>'+
      '</div>\n'+
      '</div>\n'+
      ...
      '         	</div>\n' +
      '  			<div class="tab-content conversationtxt">\n' +
      '			  	<div class="tab-pane fade in active" id="conversationwindow" style="display:none"> </div>\n' +
      '	  			<div class="tab-pane fade" id="userFile" > </div>\n' +
      '         </div>\n' +
      '         </div>\n' +

      '<div class="middle_pannel">' +
      ...
      '<div class="social-media memberdropdown">' +
      '<ul class="list-inline d-flex">' +
      //'<li><a><i class="fa fa-search" aria-hidden="true"></i></a></li>' +
      '<li><a class="CallNow"><i class="fa fa-phone" aria-hidden="true"></i></a></li>' +
      '<li><a class="VideoNow"><i class="fa fa-video-camera" aria-hidden="true"></i></a></li>' +
      '<li>' +
      '<a class="dropdown-toggle msettingicon"  data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v" aria-hidden="true"></i></a>' +
      '<ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">' +
      '<li class="dropdown-item" onClick=msgSetMarkUread();><a href="#">Mark Unread</a></li>' +
      '<li class="dropdown-item" onClick=msgSetFavourite();><a href="#"><span class="checkSdefault" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Favorite</a></li>' +
      '<li class="dropdown-item" onClick=msgSetCloseConv();><a href="#">Close Conversation</a></li>' +
      '<li class="dropdown-item" onClick=postmail();><a href="#">Post via Email</a></li>' +
      '<li class="dropdown-item" onClick="msgSetMuteConv();"><a href="#"><span class="checkmutenotify" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Mute all Notifications</a></li>' +
      '<li class="dropdown-item" onClick=msgSetDesktopConv();><a href="#"><span class="checkdesknotify" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Desktop Notifications</a></li>' +
      '<li class="dropdown-item"><a href="#">Mobile Notifications</a></li>' +
      //'<li class="dropdown-item"><a href="#">Email Notifications</a></li>' +

      '<li class="dropdown-submenu">' +
      '<a  class="dropdown-item" tabindex="-1" href="#">Email Notifications</a>' +
      ' <ul class="dropdown-menu">' +
      '<li class="dropdown-item"><a onClick=setemailnotify("15min"); href="#"><span class="checkMOneFive" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Every 15 min</a></li>' +
      '<li class="dropdown-item"><a onClick=setemailnotify("1hr"); href="#"><span class="checkMOnehr" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Every hour</a></li>' +
      '<li class="dropdown-item"><a onClick=setemailnotify("off"); href="#"><span class="checkMoff" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>off</a></li>' +
      '</ul>' +
      '</li>' +


      '<li class="dropdown-submenu">' +
      '<a  class="dropdown-item" tabindex="-1" href="#">Sound</a>' +
      ' <ul class="dropdown-menu">' +
      '<li class="dropdown-item"><a onClick=msgSetsoundConv("Default"); href="#"><span class="checksounddef" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Sound default</a></li>' +
      '<li class="dropdown-item"><a onClick=msgSetsoundConv("Ping"); href="#"><span class="checksoundping" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Sound beep1</a></li>' +
      '<li class="dropdown-item"><a onClick=msgSetsoundConv("Air"); href="#"><span class="checksoundair" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Sound beep2</a></li>' +
      '</ul>' +
      '</li>' +


      '<li class="dropdown-item"><a href="#">Add integration</a></li>' +
	   '<li class="dropdown-item"><a id="viewproLi" onClick=showUserProfile(); data-toggle="modal" data-target="#directoryViewPro" href="#">View Profile</a></li>' +
      '</ul>' +
      /*  '<a class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-ellipsis-v" aria-hidden="true"></i></a>' +
       '<div class="dropdown-menu">' +
       '<a class="dropdown-item" onClick=msgSetMarkUread();>Mark Unread</a>' +
       '<a class="dropdown-item" onClick=msgSetFavourite();>Favorite</a>' +
       '<a class="dropdown-item" onClick=msgSetCloseConv();>Close Conversation</a>' +
       '<a class="dropdown-item" onClick=postmail();>Post via Email</a>' +
       '<a class="dropdown-item" onClick="msgSetMuteConv();">Mute all Notifications</a>' +
       '<a class="dropdown-item" onClick=msgSetDesktopConv();>Desktop Notifications</a>' +
       '<a class="dropdown-item" >Mobile Notifications</a>' +
       '<a class="dropdown-item" >Email Notifications</a>' +
       '<a class="dropdown-item" >Sound</a>' +
       '<a class="dropdown-item" >Add integration</a>' +
 
       '<div class="arrow" style="left: 50%;"></div></div>' + */
      ...
      '<a href="javascript:void(0)" class="closebtn" onclick="closeNav()"><i class="fa fa-chevron-right" aria-hidden="true"></i></a>' +
		...
		'<ul class="members_list d-flex">'+
		'<li><img src="images/user.png" alt="John Doe" class="mr-3 mt-3 rounded-circle" style="width:30px;"></li>'+
		'<li><img src="images/user.png" alt="John Doe" class="mr-3 mt-3 rounded-circle" style="width:30px;"></li>'+
		'<li><img src="images/user.png" alt="John Doe" class="mr-3 mt-3 rounded-circle" style="width:30px;"></li>'+
		'<li><img src="images/user.png" alt="John Doe" class="mr-3 mt-3 rounded-circle" style="width:30px;"></li>'+
		'<li class="elips"><i class="fa fa-ellipsis-h" aria-hidden="true"></i></li>'+
  ...
  /*     '<div class="decription">	' +
      ...
      '<div class="panel chattasklist">' +
      '</div>' +
      '</div>' +
      '<div class="events">' +
      '<button class="accordion">Events</button>' +
      '<div class="panel chateventlist">' +
      '</div>' +
      '</div>' +
      ...
      '<div class="notes1 r_filessection">' +
      '<div class="events">' +
      '<img src="icons/file-s.png" alt="John Doe" class="">' +
      '<p>sample.jpg</p>' +
      ' </div>' +
      '</div>' +
      '</div>' +
      ...
      '</div>	' +
      '<div id="main">' +
      '<button class="openbtn" onclick="openNav()"><i class="fa fa-chevron-right" aria-hidden="true"></i></button>  ' +
      '</div>' +
      '</div>	' +
      '</div>\n' +
      '<div id="group_selfie_sec" class="col-md-12 col-xs-12 col-sm-12" style="display:none">' +
      '<div class="app">' +
      '<a href="#" id="group-start-camera" class="visible">Touch here to start the app.</a>' +
      '<video id="group-camera-stream"></video>' +
      '<img id="group-snap">' +
      '<p id="group-error-message"></p>' +
      '<div class="controls">' +
      '<a href="#" id="group-delete-photo" title="Delete Photo" class="disabled"><i class="fa fa-trash"></i></a>' +
      '<a href="#" id="group-take-photo" title="Take Photo"><i class="fa fa-camera"></i></a>' +
      '<a href="#" id="group-download-photo" download="selfie.png" title="Save Photo" class="disabled"><i class="fa fa-download"></i></a> ' +
      '</div>' +
      '<canvas></canvas>' +
      '</div>' +
      '<div class="form-group" id="group_selfie_save_sec" style="display:none">' +
      '<div class="col-md-12">' +
      '<button type="submit" id="group_selfie_save" class="btn btn-success">Save Changes</button>' +
      '</div>' +
      '</div>' +
      ...
      '		<div class="rightarrow"><button type="button" class="slide-toggle"><img src="././images/chat_img/settings.svg"></button></div>\n' +
      '		<div class="box" style="display:none">\n' +
      '        	<div class="box-inner"><b>Members</b></div>\n' +
      '    		</div>\n' +
      '    		</div>\n' +
      '		 <div class="whiteboardsec"/>\n' + */
      //'         <div id="conversation_sec" class="jsxc_textarea" onpaste="onpastehanlder()"/>\n' +
      '         <div id="conversation_sec" class="jsxc_textarea" onpaste="onpastehanlder()">\n' +
      '		  <div class="initwind" style="position: relative; top: 10px;"><img src="././images/chat_img/hand.svg"><span class="initwindspan">Welcome</span></div>' +
      '</div>' +
      '		  <div id="copyqoute" class="copyquote_sec"></div>\n' +
      '         <div class="jsxc_textarea_content" >\n' +
      '         <div class="jsxc_icon-set" id="audiorec_sec" style="display:none">\n' +

      '<div class=\"col-sm-1 col-xs-1\" style=\"width: 200px !important;\">\n' +
      '<button class=\"btn btn-danger btn-circle-med btn-med btn-outline\" type=\"button\" onclick="cancelAudioNotes();"><i class=\"fa fa-times\" style=\"padding: 0;\"></i></button>\n' +

      '<div id="audiotimer_display" class=\"btn-group\" style=\"padding-right:3px;font-size: 15px;\">00:00:00</div>\n' +
      '<button class=\"btn btn-primary btn-circle-med btn-med btn-outline\" type=\"button\" style="color: #fff; background-color: #5cb85c; border-color: #5cb85c;" onclick="stopRecording();"><i class=\"fa fa-check\"  style=\"padding: 0;\"></i></button>\n' +
      '</div>\n' +

      '</div>\n' +

      '         <div class="jsxc_icon-set" id="filesmily_sec">\n' +
      ' 		<div class="jsxc_editcancel" title="Cancel Edit"><i class="fa fa-times-circle" aria-hidden="true" style="font-size: 20px;color: red;"></i></div>\n' +
      // '         <div class="jsxc_audionotes"  onclick="startRecording()"><i title="Audio notes" class="fa fa-microphone" aria-hidden="true" style="font-size:18px"></i></div>\n' +

      '		  <div class="jsxc_share jsxc_sendFile">\n' +
      '               <a href="javascript:void(0);">\n' +
      '                  <img src="images/chat_img/attachment.svg" class="sideMenu-img" alt="shared files" />\n' +
      '               </a>\n' +
      '         </div>\n' +
      ...
      '         <a class="jsxc_gif" data-toggle="modal"  href="#gifshow"></a>' + gifImageHtml + '\n' +
      ...
      '<div class="mplusicon dropdown open">\n' +
      '<a id="" role="button" data-toggle="dropdown" href="#"> <img class="plusicon" src="images/chat_img/plus.svg" class="msettingtask" alt="General"> </a>\n' +

      '<ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">\n' +
      '<li><a onclick="loadcalendar();" href="#"><span class="checkdesknotify" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>New Event &nbsp;&nbsp; Alt E</a></li>\n' +
      '<li><a onclick="loadAlltask();" href="#"><span class="checkdesknotify" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>New Task	&nbsp;&nbsp;	Alt T</a></li>\n' +
      '<li><a onclick="initiatenotes();" href="#"><span class="checkdesknotify" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>New Note	&nbsp;&nbsp;	Alt N</a></li>\n' +


      /* '<li class="dropdown-submenu dropdown-menu-right">\n' +
      '<a tabindex="-1" href="#">Share Files</a>\n' +
      '<ul class="dropdown-menu">\n' +
      '<li><a href="#"><input type="file" name="files" id="fileElem" multiple="" accept="image/*">Computer</a></li>\n' +
      ...
      '         <div class="jsxc_textinput" placeholder="Type your message" onkeypress="return (this.innerText.length <= 256)" contenteditable ondrop="drop(event,bid)" ondragover="allowDrop(event)" ></div>\n' +
      '      </div>\n' +
      '      </div>\n' +
      '   </div>\n' +
	  '<div class="modal" id="directoryViewPro"></div>\n'+
      '</li>\n'+
      '';
  

jsxc.gui.template['chatWindowold'] = '<li class="jsxc_windowItem">\n' +
'   <div class="jsxc_window">\n' +
'      <div class="jsxc_bar">\n' +
'         <div class="jsxc_avatar  rightsidesec"></div>\n' +
'         <div class="jsxc_tools">\n' +
/*'         <div class="ScreenNow" title="Share Screen"><i class="fa fa-desktop" aria-hidden="true"></i></div>\n' +
'         <div class="VideoNow" title="Video Call"><i class="fa fa-video-camera" aria-hidden="true"></i></div>\n' +
'         <div class="CallNow" title="Audio Call"><i class="fa fa-phone" aria-hidden="true"></i></div>\n' +*/

//'         <div class="chatsearch" title="Search"><img src="images/chat_img/search.svg"></div>\n' +
'         <div class="CallNow" title="Audio Call"><img src="images/chat_img/call.svg"></div>\n' +
'         <div class="VideoNow" title="Video Call"><img class="chaticon" src="images/chat_img/video.png"></div>\n' +

'		  <div class="msettingicon dropdown ">\n'+
	'<a id="" role="button" data-toggle="dropdown" href="#"> <img src="images/chat_img/settings.svg" class="msettingtask" alt="General"> </a>\n'+
   '<span class="caret"></span></button>\n'+
	'<ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">\n'+	
	'<li><a onClick=msgSetMarkUread(); href="#">Mark Unread</a></li>\n'+
	'<li><a onClick=msgSetFavourite(); href="#"><span class="checkSdefault" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Favourite</a></li>\n'+
	'<li><a onClick=msgSetCloseConv(); href="#">Close Conversation</a></li>\n'+
	'<li><a onClick=postmail(); href="#">Post via Email</a></li>\n'+
	'<li><a onClick="msgSetMuteConv();" href="#"><span class="checkmutenotify" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Mute All Notifications</a></li>\n'+
	'<li><a onClick=msgSetDesktopConv(); href="#"><span class="checkdesknotify" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Desktop Notifications</a></li>\n'+
	'<li class="dropdown-submenu">\n'+
	   '<a tabindex="-1" href="#">Email Notifications</a>\n'+
	   '<ul class="dropdown-menu">\n'+
		 '<li><a onClick=setemailnotify("15min"); href="#"><span class="checkMOneFive" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Every 15 min</a></li>\n'+
		 '<li><a onClick=setemailnotify("1hr"); href="#"><span class="checkMOnehr" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Every hour</a></li>\n'+
		  '<li><a onClick=setemailnotify("off"); href="#"><span class="checkMoff" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>off</a></li>\n'+
	   '</ul>\n'+
	'</li>\n'+
	'<li class="dropdown-submenu">\n'+
	   '<a tabindex="-1" href="#">Sound</a>\n'+
		   '<ul class="dropdown-menu">\n'+
			 '<li><a onClick=msgSetsoundConv("Default"); href="#"><span class="checksounddef" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Sound default</a></li>\n'+
			  '<li><a onClick=msgSetsoundConv("Ping"); href="#"><span class="checksoundping" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Sound beep1</a></li>\n'+
			  '<li><a onClick=msgSetsoundConv("Air"); href="#"><span class="checksoundair" style="display: none;"><i class="fa fa-check" aria-hidden="true"></i></span>Sound beep2</a></li>\n'+
		   '</ul>\n'+
	'</li>\n'+
	'<li><a onClick=downloadapp(); href="#">Download App</a></li>\n';	
	
	 if(checkAdmin())
			jsxc.gui.template['chatWindow'] +='<li><a onClick=administrationWin(); href="#">Administration</a></li>\n'; 
		
	jsxc.gui.template['chatWindow'] += '</ul>\n'+
'		</div>\n'+

'<div class="Chat_Sec" title="Chat sec" style="display:none"><i class="fa-comments-o" aria-hidden="true"></i></i></div>'+
...
'            	<div class="jsxc_status-msg" />\n' +
'         	</div>\n' +
/*'  			<ul class="nav nav-tabs chatTabs">' +
'      			<li class="active" id="Parentconversationwindow"><a class ="Conversation" data-toggle="tab" href="#conversationwindow" onclick="showConversation(\'bid\')">Conversation</a></li>' +
'      			<li id="ParentuserFile"><a class ="Files" data-toggle="tab" href="#userFile" onclick="DisplaySharedFiles(\'All\',\'All\',\'Files\')">Files</a></li>' +
//'      			<li><a data-toggle="tab" href="#audio" onclick="DisplaySharedFiles(\'shared\',\'audio\')">Activities</a></li>' +
'  			</ul>' +*/
//'  <div class="covresationfile">balaji</div>\n' +
...
//'         <div id="conversation_sec" class="jsxc_textarea" onpaste="onpastehanlder()"/>\n' +
'         <div id="conversation_sec" class="jsxc_textarea" onpaste="onpastehanlder()">\n' +
'		  <div class="initwind"><img src="././images/chat_img/hand.svg"><span class="initwindspan">Welcome</span></div>'+
'</div>'+
'		  <div id="copyqoute" class="copyquote_sec"></div>\n'+
'         <div class="jsxc_textarea_content" >\n' +
'         <div class="jsxc_icon-set" id="audiorec_sec" style="display:none">\n' +
...
/*
'<div class="settingicon jsxc_msgsetting">\n'+
'<div class="dropup">\n'+
    '<button class="btn" type="button" data-toggle="dropdown"><img class="plusicon" src=././images/settings-blue.svg>\n'+
   '<span class="caret"></span></button>\n'+
    '<ul class="dropdown-menu">\n'+

	  '<li><a onclick=msgSetViewProfile(); data-toggle="modal"  href="#userviewProfile">View Profile</a></li>\n'+
	  '<li><a onclick=keyboardDetail(); data-toggle="modal" href="#keyboardinfo">Keyboard Shortcuts</a></li>\n'+
   
	'</ul>\n'+
     '</div>\n'+viewPro+KeyboardInfo+
	'</div>\n'+
*/
...
/*'<div class="PlusIcon">\n'+
		...
		'</button>\n'+
			'<ul class="dropdown-menu">\n'+
				' <li><a onclick=loadcalendar(); href="#">New Event &nbsp;&nbsp; Alt E</a></li>\n'+
				' <li><a onclick=loadAlltask() href="#">New Task	&nbsp;&nbsp;	Alt T</a></li>\n'+
				' <li><a onclick=initiatenotes(); href="#">New Note	&nbsp;&nbsp;	Alt N</a></li>\n'+
			'</ul>\n'+
		'</div>\n'+
...
'         <div class="jsxc_textinput" placeholder="Message" onkeypress="return (this.innerText.length <= 256)" contenteditable ondrop="drop(event,bid)" ondragover="allowDrop(event)" ></div>\n' +
'      </div>\n' +
'      </div>\n' +
'   </div>\n' +
'</li>\n' +
...
jsxc.gui.template['groupprofilepic'] = '<div>\n'
+'<img id="grpdb" src="assets/imgs/defaultUser.png" alt="My Image" />\n'
+'<a href="javascript:void(0);" class="btn-file btn-sm grpcamerabtn">\n'
//+'<i class="fa fa-camera"></i>\n'
+'<input type="file" name="myPhoto" id="takegroupPhoto" accept="image/*"></a>\n'
//+'<button type="button" id="unlink_dp" class="btn btn-danger btn-link btn-sm deleteBtn" style="display:none" ;><i class="fa fa-trash"></i></button>\n'

/* +'<ul class="dropdown-menu groupdropdown" role="menu" aria-labelledby="dropdownMenu">'
 ...
 */
+'</div>\n'
 +'<div class="jsxc_menu jsxc_open groupdropdown" style="display:none">\n'
+'      <ul>\n'
+'         <li id="take_selfie_group-photo">Take take</li>\n'
+'         <li id="upload_groupphoto">Upload upload</li>\n'
+'      </ul>\n'
+'</div>\n'
...
'   <div class="jsxc_caption">\n' +
'      <div class="jsxc_name" />\n' +
'      <div class="jsxc_extension" />\n' +
'      <div class="jsxc_text" />\n' +
'      <div class="jsxc_moreactions"> <span class="pull-right"><span class="fa fa-comments actionIcon buddychat"></span><span class="fa fa-phone-square actionIcon buddycall_a"></span><span class="fa fa-video-camera actionIcon buddycall_v"></span><span class="fa fa-desktop actionIcon buddyscreen"></span><span class="fa fa-calendar actionIcon buddymeeting"></span></span> </div>\n' +
'      <div class="jsxc_lastmsg">\n' +
'         <span class="badge" />\n' +
//'         <span class="jsxc_text" />\n' +
'      </div>\n' +
...
'   <div class="jsxc_menu">\n' +
'      <ul>\n' +
'         <li><a class="jsxc_rename" href="#"><span class="unread_icon" style="display:none"><i class="fa fa-check" aria-hidden="true"></i></span>Mark Unread</li>\n' +
'         <li><a class="jsxc_vcard" href="">View Profile</a></li>\n' +
'         <li><a class="jsxc_setfav" href="#"><span class="favourite_icon" style="display:none"><i class="fa fa-check" aria-hidden="true"></i></span>&nbsp;Favourite</a></li>\n' +
'         <li><a class="jsxc_desknotify" href="#"><span class="desknotify_icon" style="display:none"><i class="fa fa-check" aria-hidden="true"></i></span>&nbsp;Desktop Notification</a></li>\n' +
//'         <li><a class="jsxc_soundset" href="#">Sound</a></li>\n' +

'<li>\n'+
	'<div class="dropdown dropdown-submenu">\n'+
              '<a href="#" class="dropdown-toggle" data-toggle="dropdown">Sound</a>\n'+
             ' <ul class="dropdown-menu" style="position: relative;">\n'+
               /*  '<li class="kopie"><a href="#">Dropdown Link 5</a></li>\n'+ */
               ' <li><a class="soundmenu" id="Default" href="#">Default</a></li>\n'+
                '<li><a class="soundmenu" id="Ping" href="#">Ping</a></li>\n'+
                '<li><a class="soundmenu" id="Air" href="#">Air</a></li>\n'+
               
              '</ul>\n'+
	'</div>\n'	+	  
 '</li>\n'+
'         <li><a class="jsxc_closeconv" href="#">Close Conversation</a></li>\n' +
...
'         <li><a class="jsxc_setfav" href=""><span class="jsxc_icon"><i class="fa fa-star" aria-hidden="true"></i></span><span data-i18n="delete_buddy"></span></a></li>\n' +
 */
'      </ul>\n' +
'   </div>\n' +
...
jsxc.gui.template['boobmarkText'] ='<div class="bookmarkcls"> <div class="jsxc_avatar"></div>\n'+
      '<div class="media-body">\n'+
        '<h4 class="bookmarkname" title="jump to conversation">John Doe </h4>\n'+
		'<div class="row">\n'+
          '<div class="col-md-10">\n'+
            '<div class="message">\n'+
              '<p class="mb-0"><span class="message_span_left">Text</span><span class="message_span_time">12:00</span><br>\n'+
            '</div>\n'+
          '</div>\n'+
		  '<div class="bookmarkattr">\n'+
			'<span id="like_count">0</span>\n'+
			'<img class="msgLike" src="././images/chat_img/thumb_up.svg">\n'+
			'<img class="msgPin" src="././images/chat_img/pin.svg">\n'+
			'<img class="msgBookmark" src="././images/chat_img/bookmark.svg" title="Remove Bookmark">\n'+
			'</div>\n'+
        '</div>\n'+
      '</div>\n'+
    '</div>';
		
jsxc.gui.template['bookmarkfile'] ='<div class="bookmarkcls"><div class="jsxc_avatar"></div>\n'+
      '<div class="media-body">\n'+
        '<h4 class="bookmarkname" title="jump to conversation"> </h4>\n'+
        '<div class="row mt-1">\n'+
          '<div class="col-md-10">\n'+
            '<img src="" class="img_message"><br>\n'+
			'<span class="message_span_time">12:00</span>\n'+
          '</div>\n'+
		  '<div class="bookmarkattr">\n'+
		    '<span id="like_count">0</span>\n'+
			'<img  class="msgLike" src="././images/chat_img/thumb_up.svg">\n'+
...
jsxc.gui.template['filestore'] = '<div class="jsxc_filestore" >dddddd</div>\n' +'';
...
function checkAdmin()
{
	var logindetail = localStorage.getItem('login');
	if(logindetail != undefined)
		logindetail = JSON.parse(logindetail);
	
	if(logindetail.role_id == 1)
		return true;
	else
		return false;
}

$('.globaldd').click(function(){
	window.LastChatWindow = undefined;
	$('.searchview').remove();
	$("#clientview").empty();
	$('#clientview').load("views/globalsearch.html");
})
/*
function statusApiCallingChat(callback,ext)
{
	getMyAccAPIAccesstoken(function(val, accesstoken)
	{
		...
		{
			var usr = new Object();
			usr.domain_id = loggeduser.domain_id;
			usr.extension = ext;
			var usrdata = JSON.stringify(usr, null, '\t');
			var url = "UrappgetuserStatus";
			$.ajax({
				type: "POST",
				crossDomain: true,
				dataType: "json",
				data: usrdata,
				url: myAcc_ApiServerURL + "v1/user/"+accesstoken+"/"+url,
				async: false,
				success: function(response) {
					if (response[0].errcode == 0) 
						  callback(response[0].status_msg);
					//else
						//console.log("errmsg",response[0].errmsg);
				},
				error: function(jqXHR, textStatus, errorThrown) {
