...
export async function jsxcconnection() {
	...
	try {
		...
		if (loggedUser == undefined) {
			...
			console.log(loggedUser.sip_userid + '@' + xmpp.domain, loggedUser.sip_password, xmpp.url);
			await jsxc.start(loggedUser.sip_userid + '@' + xmpp.domain, loggedUser.sip_password);
		}

		$(document).on('connected.jsxc ()', function () {

			jsxc.xmpp.conn.addHandler(function (presence) {
					console.log('presend handler ', presence);
				},
				null, "presence");
			jsxc.xmpp.conn.addHandler(function (message) {
		...
		$(document).on('disconnected.jsxc', function () {
			...
			setTimeout(function () {
				jsxc.start(loggeduser.sip_userid + '@' + xmpp.domain, loggeduser.sip_password);
				_getAPIAccesstoken();
			}, 5000)
