...
function compareDate(a,b){
	...
	var descA = a.DTEND*1000;
						...
						var dateA = new Date(descA), dateB = new Date(descB);
						return dateA - dateB;
}
...
var obj = undefined;
if(typeof contacsarray != "undefined")	obj = contacsarray.find(data => data.ext == exten);
return obj;
}
...
let FilterdArray = [];
	...
	{	
		...
		if( (text != "") && (text != undefined))
		{
			...
			if ((mail.toUpperCase().indexOf(text) == -1) && (contName.toUpperCase().indexOf(text) == -1) && ((ext.toString()).indexOf(text) == -1) ) 	return;
    }
  ...
  contactResponse = {isContact:false, contactarray:FilterdArray}
...
export function dropDownContactList(input)
{
	...
	/* 	var moderatestatus = "images/green-active.svg";
		...
		{	
			input = input.toUpperCase();
			if ((item.email_id.toUpperCase().indexOf(input) == -1) && (item.caller_id.toUpperCase().indexOf(input) == -1) && ((item.ext.toString()).indexOf(input) == -1) ) 	return;

			div +='<li data-name=\''+teamsname+'\' data-id=\''+item.sip_login_id+'@'+xmpp.domain+'\'><span class="conlist-img"><img src='+img+'></span><span class="conlist-name">'+ContName+'</span></li>';
	...
	$("#taskContactLst li").unbind().click(function(e){
...
export function TaskCreateAssign(name, bid)
{
	...
	var temp = '<li  class="taskAvatarimg"  bid="'+bid+'">'+name+'<span class="icon_close f12"></span></li>';
	$(".taskLst").append(temp);
	...
	$('.taskLst .icon_close').click(function(){
		...
		if($(".addconlist ol").children().length == 0)
		$(".addconlist").hide();
		else
		$(".addconlist").show();
	});
...
{	
  ...
  var filterdData = GlobalData.BuddyList.filter(data => data.is_favourite == 1)
...
export function getAssineeNameList(ATTENDEE){
  ...
  var AssineeNames = "";
					...
					for(var a=0 ; a< assigneeNameList.length; a++)
					{
						...
						{
							if((assigneeNameList[a].split('@')[1]) == undefined)
								continue;
							...
							{
								var ContactDetails = GetContactDetails(assigneeNameList[a].split("@")[0].trim());
	...
	console.log("URL", GlobalData.array_taskfile[i]);
	    ...
	    for(var m=0 ; m< GlobalData.taskfileListArray.length; m++)
	   {
		...
		console.log("URL", GlobalData.taskfileListArray[m].URL);
		   if(GlobalData.taskfileListArray[m].FileName == GlobalData.array_taskfile[i])
		   {
			   console.log("URL", GlobalData.taskfileListArray[m].URL);
			   if(listofFiles == "") listofFiles = GlobalData.taskfileListArray[m].URL;
			   else listofFiles = listofFiles+","+GlobalData.taskfileListArray[m].URL;
		   }
	   } 
   }
...
if(item.includes("@conference"))
{
  ...
  if(temp != undefined)
  {
    ...
    if(temp.created_by == loggeduser.ext)
      post_msg = 1;
    
    var self = jsxc.muc;
    self.join(item, jsxc.xmpp.conn.jid, null, temp.caller_id, undefined, true, true);
...
{	
  ...
  {	
    ...
    if (temp != undefined){
      jsxc.xmpp.addBuddy(temp.sip_login_id + "@" + xmpp.domain, temp.caller_id, 
      temp.ext, temp.sip_login_id, temp.email_id);       
