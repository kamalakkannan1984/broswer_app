...
var server = app.listen(8000,'192.168.11.128', function () {  
...
async function uploadtoserver(path)
{
    ...
    let res= await axios.post("https://urmyaccount.mundio.com/v1/user/"+getAccesstokenkey+"/UrUploadFiles", formData, {
        headers: {
            ...formHeaders,
        },
    }).catch(error => {
...
async function uploadfiles(req, res)
 { 
    ...
    let filet= target_file.mv(path.join(__dirname, '/uploads', target_file.name), (err) => {if (err) throw err;	
	})
	let finalres=await uploadtoserver(path.join(__dirname, '/uploads', target_file.name));
...
app.get(api.newKey,(req, res) => {
	...
	// 	= 	req.query.Proj_ID;
	var url 	= 	ApiServerURL + "v1/user/Unifiedringmyaccountwebapiinserttokenmaster";
	var form 	= 	{	Proj_ID: projectId	};
	var postdata 	= 	JSON.stringify(form, null, '\t');
	request.post({ url: url, form: postdata, headers: ApiHeader }, function (e, r, body) {
		if(body != undefined)
		{
			body 	= jsonParser(body);
			if (body.errcode == 0) 
				res.send(body.tokenlist.AccessTokenID);
		}
	});
});
...
function centralizationToken(callback)
{
	...
	var formdata 	= 	{ SecretId:'dW5pZmllZHJpbmdhcHA=', SecretPassword:'YWJjMTIz' };
	var headersOpt 	= 	{  "Content-Type": "application/json" };

	request.post({ url: url, form: formdata, headers: headersOpt }, function (e, r, body) {
		if(body != undefined)
		{
			body 	= jsonParser(body);
	...
	switch(type) {
		...
		case api.InviteUser:
				var form = {
					...
					extn		:	req.query.extn,
					Username	: 	'Guest User',
					invite_list	: 	[{'invitedPersonname':req.query.invitedPersonname ,invitedPersonEmail: req.query.invitedPersonEmail}]
				};
				postdata 		= 	JSON.stringify(form, null, '\t');
				break;
		case api.getProfile:
...
app.get("/teamApi", function(req, res) {
	...
	switch(req.query.stype)
	{
		case api.GTeamMem:
			...
			url 		=  "https://urmyaccount.mundio.com/v1/user/"+getAccesstokenkey+"/Urappgetteaminfo";
			break;
		case api.createTeam:
			formdata = {
				company_id:req.query.company_id,
			...
			url 		=  "https://urmyaccount.mundio.com/v1/user/"+getAccesstokenkey+"/Urappcreateteaminfo";
			break;
		case api.TeamAddmem:
			formdata = {
				team_id		:	req.query.team_id,
			...
			url 		=  "https://urmyaccount.mundio.com/v1/user/"+getAccesstokenkey+"/Urappcreateteammemberinfo";
			break;
		case api.archiveTeam:
				...
				url 		=  "https://urmyaccount.mundio.com/v1/user/"+getAccesstokenkey+"/Urappteamcloseconversation";
				break;
		case api.leaveTeam:
				formdata = {
					company_id : req.query.company_id,
				...
				url 		=  "https://urmyaccount.mundio.com/v1/user/"+getAccesstokenkey+"/Urappleaveteam";
				break;
		case api.deleteTeam:
				formdata = {
					company_id : req.query.company_id,
				...
				url 		=  "https://urmyaccount.mundio.com/v1/user/"+getAccesstokenkey+"/Urappdeleteteam";
				break;
	}
		...
		request.post({ url: url, form: formdata }, function (e, r, body) {
			if(body != undefined) 
			{
				...
				if(body.Message)
				{
					getaccess(projectId, function(data)
					{
						var urlNew ="https://urmyaccount.mundio.com/v1/user/"+getAccesstokenkey;
						if(req.query.stype == api.GTeamMem)
							urlNew += "/Urappgetteaminfo";
...
app.get("/GetMongoData", function(req, res) {
	...
	if(AccesstokenCentralization == "")
	{
		...
		{
			request.post({ url: url, form: formdata, headers: ApiMongoHeader}, function (error, response, body) {
				if(body != undefined) 
				{
					body = jsonParser(body);
					res.send(body);
	...
	else
	{
		request.post({ url: url, form: formdata, headers: ApiMongoHeader}, function (error, response, body) {
			...
			if(response.statusCode == 401)
			{
				...
				{
					request.post({ url: url, form: formdata, headers: ApiMongoHeader}, function (error, response, body) {
						console.log(" regen header response code is ",response.statusCode);
						if(body != undefined) 
						{
							body = jsonParser(body);
							res.send(body);
...
app.post('/emergencyAdd',function(req,res){
	var url 	= 	ApiServerURL + "v1/user/"+getAccesstokenkey+"/Adduremergencyaddress";
	var data = req.body;
	var form = {
...
app.post('/emergencyGet',function(req,res){
	...
	var url  = 	ApiServerURL + "v1/user/"+getAccesstokenkey+"/Geturemergencyaddress";
	var data = req.body;
	var form = {
			sip_id			:	data.sip_id,
			extn_no			:	data.extn_no
...
app.post('/emergencyMod',function(req,res){
	...
	var url  = 	ApiServerURL + "v1/user/"+getAccesstokenkey+"/Modifyuremergencyaddress";
	var data = req.body;
	var form = {
			company_id		: 	data.company_id,
			customer_name	: 	data.customer_name,
...
app.post(api.fileGenrate,function(req,res){
	...
	var url = "C:/Users/b.ezhumalai/Downloads/c37fbdeced27473ea4e62ae2b3fef19a1574846868716_task.ics";
...
const googleFileClientId 		= 	'241894072484-lkhg5i556mcovif9kf4sncu59moctrsa.apps.googleusercontent.com';
...
app.get(api.googleShare,(req, res) => {	
	var url = getGoogledriveAuthenticationUrl(googleFileScopes, googleFileClientId, googleFileClientSecret);
	res.send(url);
});

app.get(api.dropShare, function (req, res) {
	var url = "https://www.dropbox.com/1/oauth2/authorize?client_id=hwxg0lbyso6tx39&response_type=token&redirect_uri=http://localhost&state=8eFwZuD1D-AAAAAAAAAAMb9Ts7QfhEbgdlJLEDruycqhwLnZCb75eVT3bR97jtyf";
	res.send(url);
});
...
app.get(api.oneDriveShare, function (req, res) {
	...
	var url ='https://login.live.com/oauth20_authorize.srf?client_id=1e5eef18-331b-4090-98ef-5183d6c8bad4&scope=files.readwrite.all offline_access&response_type=code&redirect_uri=http://localhost';
	res.send(url);
});
...
function getMsAuthenticationUrl(scopes, clientId, clientSecret) {
	...
	var MsauthEndpoint = 'https://login.microsoftonline.com/common/oauth2/v2.0/authorize?';
